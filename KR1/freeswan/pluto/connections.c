 ...
 * for more details.
 *
 * RCSID $Id: connections.c,v 1.1 2003/10/15 06:52:30 hiwu Exp $
 */
...
, bool initiator, bool aggrmode)
{
	...
	for (; d != NULL; d = d->hp_next)
	{
		...
		DBG(DBG_CONTROL, DBG_log("refine_host_connection - 5b"));
		continue;
	    }
   ...
   d = find_host_pair_connections(&c->this.host_addr, c->this.host_port
       , (ip_address *)NULL,
       st->nat_traversal & NAT_T_DETECTED ? 0 : c->that.host_port);
#else
    d = find_host_pair_connections(&c->this.host_addr, c->this.host_port
...
, const u_int8_t peer_protocol, const u_int16_t peer_port)
{
    ...
    {
#ifdef VIRTUAL_IP
	if ( (!is_virtual_connection(d)) && (!same_peer_ids(c, d, peer_id)) )
#else
...
#ifdef NAT_TRAVERSAL
	if ( (hp == NULL) && (nat_traversal_activated()) ) {
		/* find host pair with wildcard port */
		hp = find_host_pair(&c->this.host_addr, c->this.host_port, NULL, 0);
	}
#endif
...
#ifdef NAT_TRAVERSAL
void
update_host_pair(const char *why, struct connection *cnx,
	const ip_address *myaddr, u_int16_t myport ,
	const ip_address *hisaddr, u_int16_t hisport)
{
	struct connection *c;
	struct host_pair *p, *hp = cnx->host_pair;
	ip_address tmpaddr;
	...
	if ((sameaddr(&hp->me.addr, myaddr)) &&
		(hp->me.port == myport) &&
		(sameaddr(&hp->him.addr, hisaddr)) &&
		(hp->him.port == hisport)) {
		return;
	}

	if ( ((now()) - (hp->last_update)) < 2 ) {
		/* not more than one update every 2 seconds */
		return;
	}

	p = find_host_pair(myaddr, myport, hisaddr, hisport);

	{
		/* Log what we are doing */
		char iptom[ADDRTOT_BUF], iptoh[ADDRTOT_BUF];
		char iptnm[ADDRTOT_BUF], iptnh[ADDRTOT_BUF];
		addrtot(&hp->me.addr, 0, iptom, ADDRTOT_BUF);
		addrtot(&hp->him.addr, 0, iptoh, ADDRTOT_BUF);
		addrtot(myaddr, 0, iptnm, ADDRTOT_BUF);
		addrtot(hisaddr, 0, iptnh, ADDRTOT_BUF);
		log("HP: %s:%d/%s:%d -> %s:%d/%s:%d [%s]%s",
			iptom, hp->me.port, iptoh, hp->him.port, iptnm, myport,
			iptnh, hisport, why ? why : "", p ? " - moving" : "");
	}

	if (p) {
		list_rm(struct connection, hp_next, cnx, hp->connections);
		cnx->host_pair = p;
		cnx->hp_next = p->connections;
		p->connections = cnx;
		if (hp->connections == NULL
			&& !hp->initial_connection_sent) {
		...
		/* Update host_pair struct */
		hp->me.addr = *myaddr;
		hp->me.port = myport;
		setportof(htons(myport), &hp->me.addr);
		hp->him.addr = *hisaddr;
		hp->him.port = hisport;
		setportof(htons(hisport), &hp->him.addr);
		hp->last_update = now();
	}

	/* Update connections */
	for (c = hp->connections; c; c = c->hp_next) {
		if (!((sameaddr(&c->this.host_addr, myaddr)) &&
			(c->this.host_port == myport) &&
			(sameaddr(&c->that.host_addr, hisaddr)) &&
			( c->that.host_port == hisport))) {
			c->this.host_addr = *myaddr;
			c->this.host_port = myport;
			setportof(htons(myport), &c->this.host_addr);
			c->that.host_addr = *hisaddr;
			c->that.host_port = hisport;
			setportof(htons(hisport), &c->that.host_addr);
			update_states_by_connection(c);
		}
	}
}
#endif
