 ...
 * for more details.
 *
 * RCSID $Id: ipsec_doi.c,v 1.1 2003/10/15 06:52:30 hiwu Exp $
 */
	...
	unsigned long try)
{
    ...
    /* VID out */
    if (!out_generic_raw(ISAKMP_NEXT_KE, &isakmp_vendor_id_desc, &rbody,
			 dpd_vid, dpd_vid_length, "V_ID"))
	return STF_INTERNAL_ERROR;
...
main_inI1_outR1(struct msg_digest *md)
{
    ...
    struct state *st;
	struct connection *c = find_host_connection_mode(&md->iface->addr
	, ntohs(portof(&md->iface->addr)), &md->sender, md->sender_port, TRUE);
    pb_stream r_sa_pbs;
	...
	c = find_host_connection_mode(&md->iface->addr, pluto_port
		, (ip_address*)NULL, nat_traversal_activated() ? 0 : md->sender_port, TRUE);
#else
	c = find_host_connection_mode(&md->iface->addr, pluto_port
	...
	if (md->iface->ike_float == TRUE)
		nat_traversal_port_float(st, md, TRUE);
#endif
...
#ifdef NAT_TRAVERSAL
	if (md->nat_traversal_vid && nat_traversal_activated()) {
		/* reply if NAT-Traversal draft is supported */
		switch (md->nat_traversal_vid) {
			case VID_NATT_IETF_00:
				st->nat_traversal = LELEM(NAT_TRAVERSAL_IETF_00_01);
				if (!out_vendorid(ISAKMP_NEXT_VID, &md->rbody,
			...
			case VID_NATT_IETF_03:
				st->nat_traversal = LELEM(NAT_TRAVERSAL_IETF_02_03);
				if (!out_vendorid(ISAKMP_NEXT_VID, &md->rbody,
					md->nat_traversal_vid))
					return STF_INTERNAL_ERROR;
				break;
...
main_inI2_outR2(struct msg_digest *md)
{
	...
	if (st->nat_traversal) {
		nat_traversal_show_result(st->nat_traversal);
	}
...
main_inR2_outI3(struct msg_digest *md)
{
	...
	if ((st->nat_traversal & NAT_T_WITH_FLOATING_PORT) &&
		(st->nat_traversal & NAT_T_DETECTED))
		nat_traversal_port_float(st, md, FALSE);
#endif
...
aggr_inI1_outR1(struct msg_digest *md)
{
	...
	c = find_host_connection_mode(&md->iface->addr, pluto_port
#ifdef NAT_TRAVERSAL
		, (const ip_address *)NULL, nat_traversal_activated() ? 0 : md->sender_port, FALSE);
#else
    ...
    /* VID out */
    if (!out_generic_raw(ISAKMP_NEXT_KE, &isakmp_vendor_id_desc, &md->rbody,
			 dpd_vid, dpd_vid_length, "V_ID"))
	return STF_INTERNAL_ERROR;
