 ...
 * for more details.
 *
 * RCSID $Id: server.c,v 1.1 2003/10/15 06:52:30 hiwu Exp $
 */
...
create_socket(struct raw_iface *ifp, const char *v_name, int port)
{
    ...
    /* bind to device */
    if (ifp->name) {
	char _ifname[IFNAMSIZ+1];
	memset(_ifname,0,IFNAMSIZ+1);
	strncpy(_ifname, ifp->name, IFNAMSIZ);
	if (strchr(_ifname,':')) *(strchr(_ifname,':')) = '\0';
	if (setsockopt(fd, SOL_SOCKET, SO_BINDTODEVICE, _ifname,
	    strlen(_ifname)+1) < 0) {
	    log_errno((e,
	    "setsockopt SO_BINDTODEVICE in process_raw_ifaces() [%s/%s]",
	    ifp->name, _ifname));
	}
    }
#endif
	 ...
	 */
	{
...
#ifdef NAT_TRAVERSAL_FLOATED_PORT
		    if (nat_traversal_activated()) {
		...
		&& sameaddr(&q->addr, &ifp->addr))
		{
		    ...
		    { 
    			    struct iface *p;
			    for (p = interfaces; p != NULL; p = p->next) {
			    	if (streq(q->rname, p->rname) && streq(q->vname, p->vname)) {
					  p->change = IFN_KEEP;
				}	   
			    }
		    } 
