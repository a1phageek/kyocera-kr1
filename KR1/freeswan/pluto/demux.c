 ...
 * for more details.
 *
 * RCSID $Id: demux.c,v 1.1 2003/10/15 06:52:30 hiwu Exp $
 */
...
#ifdef NAT_TRAVERSAL
    , P(KE) | P(NONCE), P(VID) | P(CR) | P(NATD_R), PT(KE)
#else
...
#ifdef NAT_TRAVERSAL
    , P(KE) | P(NONCE), P(VID) | P(CR) | P(NATD_R), PT(ID)
#else
...
#ifdef NAT_TRAVERSAL
      P(SA) | P(KE) | P(NONCE) | P(ID) | P(HASH), P(VID) | P(NATD_R), PT(NONE),
#else
...
#ifdef NAT_TRAVERSAL
    , P(HASH) | P(SA) | P(NONCE), /* P(SA) | */ P(KE) | P(ID) | P(NATOA_R), PT(NONE)
#else
...
#ifdef NAT_TRAVERSAL
    , P(HASH) | P(SA) | P(NONCE), /* P(SA) | */ P(KE) | P(ID) | P(NATOA_R), PT(HASH)
#else
...
process_packet(struct msg_digest **mdp)
{
...
#ifdef NAT_TRAVERSAL
	if ((st) && (st->st_connection) && (st->st_connection->host_pair) &&
		(md->iface != st->st_connection->interface) &&
		(md->iface->ike_float == TRUE)) {
		nat_traversal_port_float(st, md, TRUE);
	}
	else if ((st) && (st->st_connection) && (st->st_connection->host_pair) &&
		(st->nat_traversal & NAT_T_DETECTED) &&
		(md->iface == st->st_connection->interface)) {
		update_host_pair("NAT-T (IKE new mapping)",
			st->st_connection,
			NULL, 0, &md->sender, md->sender_port);
	}
#endif
	...
	while (np != ISAKMP_NEXT_NONE)
	{
		...
		switch (np)
		{
			case ISAKMP_NEXT_NATD_R:
			case ISAKMP_NEXT_NATOA_R:
		...
		case ISAKMP_NEXT_ID:
		    ...
		    break;
#ifdef NAT_TRAVERSAL
		case ISAKMP_NEXT_NATD:
			np = ISAKMP_NEXT_NATD_R;  /* NAT-D relocated */
			sd = payload_descs[np];
			break;
		case ISAKMP_NEXT_NATOA:
			np = ISAKMP_NEXT_NATOA_R;  /* NAT-OA relocated */
			sd = payload_descs[np];
			break;
#endif
		default:
