 ...
 * for more details.
 *
 * RCSID $Id: state.c,v 1.1 2003/10/15 06:52:30 hiwu Exp $
 */
...
#ifdef NAT_TRAVERSAL
void
update_states_by_connection(struct connection *c)
{
	int i;
	struct state *st, *this, **h;
	...
	for (i = 0; i < STATE_TABLE_SIZE; i++)
	{
	    for (st = statetable[i]; st != NULL; ) {
			...
			if (this->st_connection == c) {
				h = state_hash(this->st_icookie, this->st_rcookie,
					&this->st_connection->that.host_addr);
				if (*h != statetable[i]) {
					if (this->st_hashchain_prev == NULL) {
						statetable[i] = this->st_hashchain_next;
					}
					else {
						this->st_hashchain_prev->st_hashchain_next = this->st_hashchain_next;
					}
					if (this->st_hashchain_next != NULL) {
						passert(this->st_hashchain_next->st_hashchain_prev == this);
						this->st_hashchain_next->st_hashchain_prev = this->st_hashchain_prev;
					}
   					this->st_hashchain_next = this->st_hashchain_prev = NULL;
					insert_state(this);
				}
			}
		}
...
#if 1
void for_each_state(void *(f)(struct state *))
{
	struct state *st;
	int i;
	for (i=0; i<STATE_TABLE_SIZE; i++)
