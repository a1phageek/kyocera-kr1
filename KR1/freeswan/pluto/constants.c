 ...
 * for more details.
 *
 * RCSID $Id: constants.c,v 1.1 2003/10/15 06:52:30 hiwu Exp $
 */
...
static enum_names payload_names_nat_d =
	{ ISAKMP_NEXT_NATD, ISAKMP_NEXT_NATOA, payload_name_nat_d, NULL };
enum_names payload_names =
    { ISAKMP_NEXT_NONE, ISAKMP_NEXT_NATOA_R, payload_name, &payload_names_nat_d };
#else
enum_names payload_names =
...
static enum_names enc_udp_mode_names =
    { ENCAPSULATION_MODE_UDP_TUNNEL, ENCAPSULATION_MODE_UDP_TRANSPORT, enc_udp_mode_name, NULL };
enum_names enc_mode_names =
    { ENCAPSULATION_MODE_TUNNEL, ENCAPSULATION_MODE_TRANSPORT, enc_mode_name, &enc_udp_mode_names };
#else
enum_names enc_mode_names =
...
/* Vendor IDs */
const char dpd_vid[] = {0xAF, 0xCA, 0xD7, 0x13, 0x68, 0xA1, 0xF1, 
   0xC9, 0x6B, 0x86, 0x96, 0xFC, 0x77, 0x57, 0x01, 0x00};
const int dpd_vid_length = sizeof(dpd_vid);
