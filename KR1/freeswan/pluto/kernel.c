 ...
 * for more details.
 *
 * RCSID $Id: kernel.c,v 1.1 2003/10/15 06:52:30 hiwu Exp $
 */
...
setup_half_ipsec_sa(struct state *st, bool inbound)
{
	...
	*said_next = said;
	...
	if (st->nat_traversal & NAT_T_DETECTED) {
		switch (st->nat_traversal & NAT_TRAVERSAL_METHOD) {
			...
			case LELEM(NAT_TRAVERSAL_IETF_02_03):
				...
				break;
			default:
				loglog(RC_LOG_SERIOUS, "unknown NAT-Traversal method %ld",
					(long)(st->nat_traversal & NAT_TRAVERSAL_METHOD));
				goto fail;
				break;
		}
...
#ifdef NAT_TRAVERSAL
void update_ipsec_sa (struct state *st USED_BY_KLIPS)
{
#ifdef KLIPS
	if (IS_IPSEC_SA_ESTABLISHED(st->st_state)) {
		/* 
		 * This is not perfect -- TODO: really update instead of
		 * delete & add.
		 */
		(void) teardown_half_ipsec_sa(st, FALSE);
		(void) teardown_half_ipsec_sa(st, TRUE);
    	setup_half_ipsec_sa(st, TRUE);
    	setup_half_ipsec_sa(st, FALSE);
		(void) sag_eroute(st, ERO_REPLACE, "restore");
	}
		...
		DBG_log("assert failed at %s:%d", __FILE__, __LINE__);
	}
	return;
#else /* !KLIPS */
