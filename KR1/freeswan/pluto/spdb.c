 ...
 * for more details.
 *
 * RCSID $Id: spdb.c,v 1.1 2003/10/15 06:52:30 hiwu Exp $
 */
...
#else
				/* Always UDP_TUNNEL as long as Transport Mode has
				 * security concerns
				 *
	...
	switch (a.isaat_af_type)
	{
		...
		switch (val) {
			...
			case ENCAPSULATION_MODE_TRANSPORT:
				...
				break;
			case ENCAPSULATION_MODE_UDP_TRANSPORT:
#ifndef I_KNOW_TRANSPORT_MODE_HAS_SECURITY_CONCERN_BUT_I_WANT_IT
				loglog(RC_LOG_SERIOUS,
				...
				break;
#endif
			case ENCAPSULATION_MODE_UDP_TUNNEL:
				if (st->nat_traversal & NAT_T_DETECTED) {
					attrs->encapsulation = val - ENCAPSULATION_MODE_UDP_TUNNEL + ENCAPSULATION_MODE_TUNNEL;
				}
				else {
					loglog(RC_LOG_SERIOUS,
						"%s must only be used if "
