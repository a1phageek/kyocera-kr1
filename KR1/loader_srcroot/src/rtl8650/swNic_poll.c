...
* All rights reserved.
* 
* $Header: /home/cvsroot/uClinux-dist/loader_srcroot/src/rtl8650/swNic_poll.c,v 1.4 2004/07/14 02:16:09 yjlou Exp $
*
...
int32 swNic_receive_t(void** input, uint32* pLen)
{
    struct pktHdr * pPkthdr;
    int32 pkthdr_index;
    int32 mbuf_index;
...
//    {
//    	printf(" Des[%d] is %X\n",counter,(RxPkthdrDesc[counter] & DESC_OWNED_BIT) == DESC_RISC_OWNED);
//    } 
    ...
    {
        ASSERT_ISR(currRxPkthdrDescIndex < num_of_rx_pkthdr_desc);
   //     printf("swNic_receive_t --1\n");
        /* Fetch pkthdr */
//        printf(" DesGet[%d] is %X\n",currRxPkthdrDescIndex,(RxPkthdrDesc[currRxPkthdrDescIndex] & DESC_OWNED_BIT) == DESC_RISC_OWNED);
        pPkthdr = (struct pktHdr *) (RxPkthdrDesc[currRxPkthdrDescIndex] & 
                                            ~(DESC_OWNED_BIT | DESC_WRAP));
        ...
        /* Increment counter */
//        printf("rxPktCounter is %d\n",rxPktCounter);
        rxPktCounter++;
//printf("swNic_receive_t\n");
        /* Output packet */
        *input = pPkthdr->ph_mbuf->m_data;
        *pLen = pPkthdr->ph_len - 4;
...
int32 swNic_send_t(void * output, uint32 len)
{
    struct pktHdr * pPkthdr;
    int i,counter;
...
//    {
//    	printf(" Des[%d] is %X\n",counter,(*(volatile uint32 *)&TxPkthdrDesc[counter] & DESC_OWNED_BIT));
//    }
    ASSERT_CSP( ((int32) TxPkthdrDesc[currTxPkthdrDescIndex] & DESC_OWNED_BIT) == DESC_RISC_OWNED );
    flag ++;
    /* Fetch packet header from Tx ring */
    ...
    for(i=(40 + UIP_LLH_LEN); i < len; i++) {
      pPkthdr->ph_mbuf->m_data[i] = uip_appdata[i - 40 - UIP_LLH_LEN];
    }
    /* Set destination port */
    if ( (pPkthdr->ph_mbuf->m_data[0] & 1) || 
            (arpResolve(pPkthdr->ph_mbuf->m_data, &pPkthdr->ph_portlist) != 0) )
    ...
    /* Set TXFD bit to start send */
    REG32(CPUICR) |= TXFD;
//    printf("pPkthdr->ph_len is %d\n",pPkthdr->ph_len);
//    printf("pPkthdr->ph_flags is %X\n",pPkthdr->ph_flags );
//    printf("pPkthdr->ph_proto is %X\n",pPkthdr->ph_proto );
//    printf("pPkthdr->ph_reason is %X\n",pPkthdr->ph_reason );
//     printf("pPkthdr->ph_mbuf->m_flags is %X\n",pPkthdr->ph_mbuf->m_flags );
//    printf("pPkthdr %X\n",pPkthdr);
//    printf(" Des is %X\n",*(volatile uint32 *)&TxPkthdrDesc[currTxPkthdrDescIndex]);
//    printf(" Des& is %X\n",(*(volatile uint32 *)&TxPkthdrDesc[currTxPkthdrDescIndex] & DESC_OWNED_BIT));
//    printf("CPUIISR is %X\n",REG32(CPUIISR));
//    printf("CPUICR is %X\n",REG32(CPUICR));
...
//    		    printf(" Des is %X\n",*(volatile uint32 *)&TxPkthdrDesc[currTxPkthdrDescIndex]);
//    		    printf(" Des& is %X\n",(*(volatile uint32 *)&TxPkthdrDesc[currTxPkthdrDescIndex] & DESC_OWNED_BIT));
//    		    printf("1CPUIISR is %X\n",REG32(CPUIISR));
//    		    printf("1CPUICR is %X\n",REG32(CPUICR));
//    		    for(i=0;i<74;i++)
//    		    	printf("pPkthdr->ph_mbuf->m_data is %X\n",pPkthdr->ph_mbuf->m_data[i]);
//    		    REG32(CPUICR) |= TXFD;
 		   // break;
                }
//    printf("txPktCounter is %d\n",txPktCounter);
//    printf("Send\n");
    txPktCounter++;
    if ( ++currTxPkthdrDescIndex == num_of_tx_pkthdr_desc )
        currTxPkthdrDescIndex = 0;
