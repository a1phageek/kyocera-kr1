...
#if defined(CONFIG_RTL865X) || defined(CONFIG_RTL8186)//new
#undef HARD_RESET_NOW
#define HARD_RESET_NOW() panic("Not yet implemented\n");
#endif


#ifdef CONFIG_SIMULATOR	//new
#undef HARD_RESET_NOW
#define HARD_RESET_NOW() panic("HARD_RESET_NOW on Simulator\n");
#endif
...
#if defined( CONFIG_SIMULATOR ) || defined( CONFIG_RTL865X ) || defined(CONFIG_RTL8186)
#if 1 
extern void *__romfs_rd_start;
extern void *__romfs_rd_end;
#define FIXED_ROMARRAY	((unsigned long)&__romfs_rd_start)
#else
extern void *__romfs_rd_start;
#define FIXED_ROMARRAY		0xbfcb8b80//((unsigned long)&__romfs_rd_start)
#endif
#endif
...
#if defined(CONFIG_ARCH_DSC21) || defined(CONFIG_ARCH_ATMEL) || defined(CONFIG_ARCH_SAMSUNG)
...
#ifdef	CONFIG_SIMULATOR //new
	panic("HARD_RESET_NOW on uclinux\n");
#else    
	HARD_RESET_NOW();
#endif
  }
...
int __init blkmem_init( void )
{
  ...
  for(i=0;i<arenas;i++) {
    if (arena[i].length == -1)
// arena[i].length = &__romfs_rd_end-&__romfs_rd_start;
...
//    arena[i].address = (unsigned long) ioremap_nocache((int) arena[i].address, arena[i].length);
    arena[i].address = (unsigned long)arena[i].address|0xa0000000;
  }
