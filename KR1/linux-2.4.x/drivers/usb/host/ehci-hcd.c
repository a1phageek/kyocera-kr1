...
done2:
	...
	{
		// for ALI USB in TEST MODE 
		// PORT 0 & PORT 2
		/*
		u32	temp;
			
		printk("echi_start() testing\n");
		printk("base+40h = %x, addr=%x\n", ehci->regs->configured_flag, &ehci->regs->configured_flag);	
		readl (&ehci->regs->configured_flag);
		printk("read base+40h = %x, ret=%x\n", ehci->regs->configured_flag, readl(&ehci->regs->configured_flag));	
		
		
		printk("base+44h 1= %x\n",readl (&ehci->regs->port_status[0]));
		printk("base+44h 2= %x\n",ehci->regs->port_status[0]);
		
		temp = readl (&ehci->regs->port_status [0]);
		temp |= 0x00040000;
		writel (temp, &ehci->regs->port_status [0]);
		
		printk("in=%x, read base+44h = %x\n",temp, readl (&ehci->regs->port_status[0]));
		
		printk("base+4Ch 1= %x\n",readl (&ehci->regs->port_status[2]));
		printk("base+4Ch 2= %x\n",ehci->regs->port_status[2]);
		temp = readl (&ehci->regs->port_status [2]);
		temp |= 0x00040000;
		writel (temp, &ehci->regs->port_status [2]);
		
		printk("in=%x, read base+4Ch = %x\n",temp, readl (&ehci->regs->port_status[2]));
		*/
		
		
		// for NEC in TEST MODE
		// PORT 0 & PORT 1
		/*
		u32	temp;
		...
		printk("base+48h 1= %x\n",readl (&ehci->regs->port_status[1]));
		printk("base+48h 2= %x\n",ehci->regs->port_status[1]);
		temp = readl (&ehci->regs->port_status [1]);
		temp |= 0x00040000;
		writel (temp, &ehci->regs->port_status [1]);
		
		printk("in=%x, read base+48h = %x\n",temp, readl (&ehci->regs->port_status[1]));
		*/
	}
