 ...
 * 2001/03/24 td/ed hashing to remove bus_to_virt (Steve Longerbeam);
 	rtl_map_single (db)
 * 2001/03/21 td and dev/ed allocation uses new rtl_pool API (db)
 * 2001/03/07 hcca allocation uses rtl_alloc_consistent (Steve Longerbeam)
 *
...
#define RTL_R32(addr)  swab32((*(VLONG*)(addr)))
#define RTL_W32(val,addr) (*(VLONG*)(addr))=swab32(val)
#undef readl
#undef writel
#define readl(x) RTL_R32(x)
#define writel(v,addr) RTL_W32(v,addr)
#else
#define RTL_R32(addr) readl(addr) 
#define RTL_W32(val,addr) writel(val,addr) 
...
dma_addr_t rtl_map_single(void *ptr, size_t size, int direction)
{
...
#if 0
	return (dma_addr_t )(( u32) ptr|0xa0000000);
#else
 ...
 * must match what was provided for in a previous rtl_map_single call.  All
 * other usages are undefined.
 *
 * After this call, reads by the cpu to the buffer are guarenteed to see
 * whatever the device wrote there.
 */
void rtl_unmap_single(dma_addr_t dma_addr,
				    size_t size, int direction)
{

	dma_cache_wback_inv((unsigned long)dma_addr, size);
	if (direction == PCI_DMA_NONE)
		out_of_line_bug();
...
static void urb_free_priv (struct ohci *hc, urb_priv_t * urb_priv)
{
	...
	if (last >= 0) {
		...
		/* unmap data buffer */
		if (len && td->data_dma)
			rtl_unmap_single ( td->data_dma, len, dir);
		...
		for(i = 0; i < NUM_EDS; i++) {
  			...
  			if (ed->state != ED_NEW) {
  				if (ed->state == ED_OPER) {
					...
					dbg ("driver usb dev %d ed 0x%x unfreed URB",
						 usb_dev->devnum, i);
					ep_unlink (ohci, ed);
				}
  				ep_rm_ed (usb_dev, ed);
...
static struct usb_operations sohci_device_operations = {
	sohci_alloc_dev,
	sohci_free_dev,
...
#if  0
	printk("hwINFO %08x , hwCBP %08x hwNextTD %08x hwBE %08x index %x\n",
			td->hwINFO,td->hwCBP,td->hwNextTD,td->hwBE,td->index);
	printk("hwINFO %08x , hwCBP %08x hwNextTD %08x hwBE %08x index %x\n",
			le32_to_cpu(td->hwINFO),le32_to_cpu(td->hwCBP),
			le32_to_cpu(td->hwNextTD),le32_to_cpu(td->hwBE),td->index);
#endif
...
static int rh_send_irq (ohci_t * ohci, void * rh_data, int rh_len)
{
	...
	if (num_ports > MAX_ROOT_PORTS) {
		err ("bogus NDP=%d for OHCI usb", num_ports);
		err ("rereads as NDP=%d", readl (&ohci->regs->roothub.a) & RH_A_NDP);
...
static int hc_start (ohci_t * ohci)
{
	...
	fminterval |= (0x2778) ;
	fminterval = 0x27782edf;

	printk ( "write minterval %x swap %x\n",fminterval,le32_to_cpu(fminterval)),
	RTL_W32(fminterval, &ohci->regs->fminterval);	
	printk ( "read fminterval %x swap %x\n",readl(&ohci->regs->fminterval),
		le32_to_cpu(readl(&ohci->regs->fminterval))	
			);
	//ohci->HcRegs->PeriodicStart = (0x2edf*9)/10;
	RTL_W32 ( (0x2edf*9)/10,&ohci->regs->periodicstart);
...
static void hc_interrupt (int irq, void * __ohci, struct pt_regs * r)
{
	...
	if (ints & OHCI_INTR_UE) {
		ohci->disabled++;
		err ("OHCI Unrecoverable Error, controller usb disabled");
...
static ohci_t * __devinit hc_alloc_ohci (void * mem_base)
{
	...
	ohci->hcca = rtl_alloc_consistent (sizeof *ohci->hcca,
			&ohci->hcca_dma);
        if (!ohci->hcca) {
                kfree (ohci);
                return NULL;
...
//	rtl_set_drvdata(dev, ohci);
	...
	if (!ohci->bus) {
		rtl_free_consistent (sizeof *ohci->hcca,
...
//	rtl_set_drvdata(ohci->ohci_dev, NULL);
	if (ohci->bus) {
...
static void hc_restart (ohci_t *ohci)
{
	...
	if ((temp = hc_reset (ohci)) < 0 || (temp = hc_start (ohci)) < 0) {
		err ("can't restart usb, %d",temp);
	} else
		dbg ("restart usb completed");
}
...
static int __init ohci_init ()
{
	void *mem_base=(void*)0xbd000000;
	int status;
	
	status = *(int*)0xbd012064;
#if  BIG 
	*(int*)0xbd012064=status|0x1;
#else
	*(int*)0xbd012064=status&~1;
#endif
	status = *(int*)0xbd012064;
	printk("USB configurate change %x \n",status);
	status = hc_found_ohci (1, mem_base);
	return status;
} 
...
static void __exit ohci_remove ()
{
	ohci_t * ohci=rtl865x_ohci;
#ifdef	DEBUG
	ohci_dump (ohci, 1);
#endif
