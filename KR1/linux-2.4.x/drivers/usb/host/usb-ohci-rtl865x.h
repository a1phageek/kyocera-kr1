...
#define OHCI_QUIRK_SUCKYIO	0x02		/* NSC superio */
	...
	u8		pci_latency;
	struct rtl_pool	*td_cache;
	struct rtl_pool	*dev_cache;
...
{
	ohci->td_cache = rtl_pool_create ("ohci_td", 
		sizeof (struct td),
		32 /* byte alignment */,
	...
	if (!ohci->td_cache)
		return -ENOMEM;
	ohci->dev_cache = rtl_pool_create ("ohci_dev",
...
{
	if (ohci->td_cache) {
		rtl_pool_destroy (ohci->td_cache);
		ohci->td_cache = 0;
	}
	if (ohci->dev_cache) {
		rtl_pool_destroy (ohci->dev_cache);
		ohci->dev_cache = 0;
	}
}
...
td_alloc (struct ohci *hc, int mem_flags)
{
	...
	td = rtl_pool_alloc (hc->td_cache, mem_flags, &dma);
	if (td) {
		td->td_dma = dma;

		/* hash it for later reverse mapping */
		if (!hash_add_td (hc, td)) {
			rtl_pool_free (hc->td_cache, td, dma);
			return NULL;
		}
	}
	return td;
...
{
	hash_free_td (hc, td);
	rtl_pool_free (hc->td_cache, td, td->td_dma);
}
...
dev_alloc (struct ohci *hc, int mem_flags)
{
	...
	dev = rtl_pool_alloc (hc->dev_cache, mem_flags, &dma);
	if (dev) {
...
{
	rtl_pool_free (hc->dev_cache, dev, dev->dma);
}
