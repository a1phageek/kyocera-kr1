...
#define GPIO_B2_USB_LED1          0x00080000    // 2. USB 1 LED [out]
#define GPIO_B3_USB_LED2          0x00020000    // 3. USB 2 LED [out]

#define USB_PORT1                 1 // physical port ID, [ALi PCB: 2], [NEC PCB 1].
#define USB_PORT2                 0 // physical port ID

#define PORT_REMOVE               0x100
#define PORT_INSERT               0x501

// ------------------------------------------------ added by Ed Lai 2005/06/29
...
static int phy_port = -1;

static void set_USB_led(int port, int action)
{
	unsigned long state = 0;

/*	
	if(USB_PORT1 == port)
		state = GPIO_B2_USB_LED1;
	else if(USB_PORT2 == port)
		state = GPIO_B3_USB_LED2;
...
static void hub_irq(struct urb *urb)
{
	...
	//info("<hub_irq>: hub->dev->ttport=%d",hub->dev->ttport);
...
static int usb_hub_configure(struct usb_hub *hub, struct usb_endpoint_descriptor *endpoint)
{
	...
	info("%d port%s detected", hub->descriptor->bNbrPorts, (hub->descriptor->bNbrPorts == 1) ? "" : "s");
        printk("%d port%s detected", hub->descriptor->bNbrPorts, (hub->descriptor->bNbrPorts == 1) ? "" : "s");
	{
		u16 hubchar=hub->descriptor->wHubCharacteristics;
		hub->descriptor->wHubCharacteristics=((hubchar>>8)&0xff)|((hubchar<<8)&0xff00);		
	...
	info("<usb_hub_config>: FILL_INT_URB(hub_irq)");
		       ...
		       const struct usb_device_id *id)
{
	...
	//info("<hub_probe>: dev->ttport=%d",dev->ttport);
...
static void hub_disconnect(struct usb_device *dev, void *ptr)
{
	...
	//info("<hub_disconnect>: dev->ttport=%d",dev->ttport);
...
{
	//info("<hub_ioctl>: hub->ttport=%d",hub->ttport);
	...
	if (portsts) {
		...
		if (ret < 0)
			...
			*change = le16_to_cpu(portsts->wPortChange); 
			...
			//info("<usb_hub_port_status>:port %d, portstatus %x, change %x, %s", port + 1,
			//	*status, *change, portspeed(*status));
					...
					u16 portstatus, u16 portchange)
{
	...
	for (i = 0; i < HUB_PROBE_TRIES; i++) {
		...
		// get physical port info 1st.
		//dev->physical_port_id = phy_port;
		//phy_port = -1;
...
static void usb_hub_events(void)
{
	...
	while (1) {
		...
		for (i = 0; i < hub->descriptor->bNbrPorts; i++) 
		{
			...
			{
				int flag = 0;
				if(portstatus == PORT_REMOVE){
					//set_USB_led(i, 0);
					printk("[ED] dont forget midify this file ,%s \n",__FILE__);
				}
				else if(portstatus == PORT_INSERT)				
				{
					//set_USB_led(i, 1);
					flag = 1;
