...
#include "pl2303.h"
// Added by Ed Lai 2005/05/18 ? why cannot using kernel menuconfig to enable it ??
...
// Modify by Ed.Lai (chang generic_device_ids array size from 2 to 6 )
...
static int generic_write (struct usb_serial_port *port, int from_user, const unsigned char *buf, int count)
{
	...
	if (serial->num_bulk_out) {
		...
		printk ("%s: %s: %d : count(%s)"__FILE__, __FUNCTION__,__LINE__, count);
		#else
...
static void generic_read_bulk_callback (struct urb *urb)
{
	...
	#ifdef USB_SERIAL_DEBUG
	printk ("%s: %s: %d :urb->actual_length(%d)"__FILE__, __FUNCTION__,__LINE__, urb->actual_length);
	#else
			       ...
			       const struct usb_device_id *id)
{
	...
	list_for_each (tmp, &usb_serial_driver_list) {
		...
		printk("[%s::%d] idVendor(%04x) idProduct(%04x)\n", __FILE__,__LINE__,dev->descriptor.idVendor,dev->descriptor.idProduct); //ed2005

		id_pattern = usb_match_id(dev, interface, type->id_table);

		if (id_pattern != NULL) {
			dbg("descriptor matches"); printk("descriptor matches \n"); //ed2005
			found = 1;
			break;
		}
	}
	if (!found) {
		/* no match */
		dbg("none matched"); printk("none matched \n"); //ed2005
		return(NULL);
	}
...
static int __init usb_serial_init(void)
{
	...
	// ? ------------------- ------------------------ -------------------- $ EdLai modify Start
	generic_device_ids[1].idVendor = 0x0c88;  // PID/VID for 1xEvDO Card usb device
	generic_device_ids[1].idProduct = 0x17da;
	generic_device_ids[1].match_flags = USB_DEVICE_ID_MATCH_VENDOR | USB_DEVICE_ID_MATCH_PRODUCT;

	generic_device_ids[2].idVendor = 0x0c88;  // PID/VID for 1xEvDO cell phone's usb device
	generic_device_ids[2].idProduct = 0x005a;
	generic_device_ids[2].match_flags = USB_DEVICE_ID_MATCH_VENDOR | USB_DEVICE_ID_MATCH_PRODUCT;

	/*
	generic_device_ids[3].idVendor = 0x0619;  // PID/VID for PHS J88 usb device
	generic_device_ids[3].idProduct = 0x0201;
	generic_device_ids[3].match_flags = USB_DEVICE_ID_MATCH_VENDOR | USB_DEVICE_ID_MATCH_PRODUCT;

	generic_device_ids[4].idVendor = 0x067b;  // PID/VID for Ed's USB to serial device
	generic_device_ids[4].idProduct = 0x2303;
	generic_device_ids[4].match_flags = USB_DEVICE_ID_MATCH_VENDOR | USB_DEVICE_ID_MATCH_PRODUCT;

	generic_device_ids[5].idVendor = 0x0403;  // PID/VID for Peter's USB to serial device
	generic_device_ids[5].idProduct = 0x6001; // ?? Why also fork ftdi_sio.c FTDI Serial Conv. 
	generic_device_ids[5].match_flags = USB_DEVICE_ID_MATCH_VENDOR | USB_DEVICE_ID_MATCH_PRODUCT;
	
	generic_device_ids[6].idVendor = 0x1410;  // PID/VID for Peter's USB to serial device
	generic_device_ids[6].idProduct = 0x1110; // ?? Why also fork ftdi_sio.c FTDI Serial Conv. 
