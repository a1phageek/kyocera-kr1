...
//-------------------------- Added by Ed Lai ,2005/05/18
static void usb_dumper_time (int line){
    struct timeval now; 
    do_gettimeofday(&now); 
    printk(" [ED]%s ::%d :: gettime : %9i.%06i ( sec.microsec )\n", __FILE__, line ,(int) now.tv_sec, (int)now.tv_usec);
}
//--------------------------------------
/**
	     ...
	     const struct usb_device_id *id)
{
	...
	//printk("[match_id] I am coming =usb_match_id= \n"); //ed2005
	//info("<usb_match_id> device: idVendor=%d, idProduct=%d ", dev->descriptor.idVendor,dev->descriptor.idProduct);
	...
	for (; id->idVendor || id->bDeviceClass || id->bInterfaceClass ||
	       id->driver_info; id++) {
	//	printk("[match_id] vendor(%04x<->%04x) product(%04x<->%04x)\n",id->idVendor, dev->descriptor.idVendor,id->idProduct, dev->descriptor.idProduct); //ed2005
...
static void call_policy_interface (char *verb, struct usb_device *dev, int interface)
{
	...
	// Add USB physical port info
	//envp [i++] = scratch;
	//scratch += sprintf (scratch, "USBPORT=%d",dev->physical_port_id);
...
int usb_submit_urb(struct urb *urb)
{
	...
	{
		#ifdef USB_DEBUG
		printk("usb_submit_urb: calling op->submit_urb\n");
		#endif
	...
	{
		#ifdef USB_DEBUG
		printk("usb_submit_urb: rturn -ENODEV\n");
		#endif
		return -ENODEV;
	}
}
...
void usb_disconnect(struct usb_device **pdev)
{
	...
	*pdev = NULL;
	...
	info("USB disconnect on device %s-%s address %d, <port=%d>",
			dev->bus->bus_name, dev->devpath, dev->devnum, dev->ttport);
#endif
...
void usb_connect(struct usb_device *dev)
{
	 ...
	 */
	 ...
	 //info("<usb_connection>: dev->ttport=%d",dev->ttport);
...
int usb_new_device(struct usb_device *dev)
{
	...
	usb_dumper_time(__LINE__); // Ed Lai 2005/05/18
	...
	usb_dumper_time(__LINE__); //ed2005
	...
	dev->descriptor.iSerialNumber&=0x0000ffff;
