 ...
 *
 * Patch to 2.4.32??
 * RTL Tony patch for NEC crash ??
 * hc_interrupt fuction ??
 *
 * [ GST Change Log ]
 *
 *  20051128 $Ed Lai$
 *	CHG: patched to kernel 2.4.31 form uxlinux 2.4.26
 *	CHG: Since Peter's priority program can detect WAN side dis-connect, wlan card have first priority now!!  
 *
*/
...
//ohci_t * global_ohci=NULL; // ed 2.4.31 remove 

/*-------------------------------------------------------------------------*
 * URB support functions 
 *-------------------------------------------------------------------------*/ 
#if 1 //******************************** Added by Ed Lai 2005/12/04
#define MAX_bogus_NDP_counter 50
#define SYSTEM_RUNNING_TIME 5000	// Added by Ed 2005/0705
int	bogus_NDP_counter = 0 ;
#endif //*******************************
 
// ? ----------------------- ed 2005 -------------------------------------- ?
static void usb_dumper_ohci_regs (ohci_t *controller){
 struct ohci_regs *regs = controller->regs;
	printk (" ..:: dump ohci_reg ( un_parsed && using readl )::..\n");
	printk ("   OHCI_Reg    | MSB(31) LSB \n");
	printk (" ============================== \n");
	printk (" HC revision   : 0x%08x \n" , (int)readl (&regs->revision));
	printk (" HC control    : 0x%08x \n" , (int)readl (&regs->control));
	printk (" HC cmdstatus  : 0x%08x \n" , (int)readl (&regs->cmdstatus));
    printk (" HC intrstatus : 0x%08x \n" , (int)readl (&regs->intrstatus));
    printk (" HC intrenable : 0x%08x \n" , (int)readl (&regs->intrenable));
	printk (" HC intrdisable: 0x%08x \n" , (int)readl (&regs->intrdisable));
        /* memory pointers */
    printk (" HC hcca       : 0x%08x \n" ,(int)readl (&regs->hcca));
    printk (" HC ed_periodcurrent :  0x%08x \n" ,(int)readl (&regs->ed_periodcurrent));
    printk (" HC ed_controlhead   :  0x%08x \n" ,(int)readl (&regs->ed_controlhead));
    printk (" HC ed_controlcurrent:  0x%08x \n" ,(int)readl (&regs->ed_controlcurrent));
    printk (" HC ed_bulkhead      :  0x%08x \n" ,(int)readl (&regs->ed_bulkhead));
    printk (" HC ed_bulkcurrent   :  0x%08x \n" ,(int)readl (&regs->ed_bulkcurrent));
    printk (" HC donehead         :  0x%08x \n" ,(int)readl (&regs->donehead));
        /* frame counters */
	printk (" HC fminterval       :  0x%08x \n" ,(int)readl (&regs->fminterval));
    printk (" HC fmremaining      :  0x%08x \n" ,(int)readl (&regs->fmremaining));
    printk (" HC fmnumber         :  0x%08x \n" ,(int)readl (&regs->fmnumber));
    printk (" HC periodicstart    :  0x%08x \n" ,(int)readl (&regs->periodicstart));
    printk (" HC lsthresh         :  0x%08x \n" ,(int)readl (&regs->lsthresh));
        /* Root hub ports */
	printk (" HC roothub.a        :  0x%08x \n" ,(int)readl (&regs->roothub.a));
    printk (" HC roothub.b        :  0x%08x \n" ,(int)readl (&regs->roothub.b));
    printk (" HC roothub.status   :  0x%08x \n" ,(int)readl (&regs->roothub.status));
	printk (" HC roothub.portstatus: 0x%08x \n" ,(int)readl (&regs->roothub.portstatus));
 
	printk(" .... dump ohci_reg  ( un_parsed && using readl )....\n");        
}
 

static void usb_dumper_endian (){ 
	int e = 1; 
	if ( *(char*)&e == 1 ) 
		printk (" .... ! Litter Endian ! .... \n"); 
	else
		printk (" .... ! Big Endian ! .... \n");
}

static void usb_dumper_pciHead (struct pci_dev *dev){
	int i,j; 
	// 0-255 => 0-248(249)
	printk (" ..:: dump pci header ::.. \n");
	for (i=0; i<64/16; i ++){
		printk ("0x%02x: ", i*16 );
		for (j=0; j<16; j += sizeof(u8)){
			u8 tmp;
			pci_read_config_byte(dev,j+16*i,&tmp);
			printk (" %02x", tmp);
		}printk ("\n");
	}
	printk(" ::.. dump pci header ..:: \n");      	
...
static void usb_enable_pci_slot1_frame (int line){

	//printk("[ED] enable_pci_slot1_frame at %d \n" ,line);	 // Added by Ed Lai 2005/06/29

	REG32(0xbd01200c)&=(~0x00020000); 
	REG32(0xbd012010)|=0x00020000; 
	REG32(0xbd012014)&=(~0x00020000);
}
// ? ----------------------- ed 2005 -------------------------------------- ?
	...
	struct urb * urb, int index)
{
	...
	// usb_enable_pci_slot1_frame(__LINE__);	// Added by Ed Lai 2005/06/29
	...
	//by peterjiang
#if 0
	printk("td_fill(%p): hwINFO(%08x) hwCBP(%08x) hwNextTD(0x%08x) hwBE(0x%08x) hwPSW(0x%04x)\n", td, le32_to_cpup(&td->hwINFO), le32_to_cpup(&td->hwCBP),le32_to_cpup(&td->hwNextTD),  le32_to_cpup(&td->hwBE), le16_to_cpup(&td->hwPSW[0]));
	{
		char *test;
		int i;
		printk("            content: ");
		for(i=0, test=(char*)(KSEG1ADDR(data)); i<len; i++)
		{
			printk("%02x ", test[i]);
		}
		printk("\n");
...
#if 0
	//peterjiang
	printk("td     (%p): hwINFO(%08x) hwCBP(%08x) hwNextTD(0x%08x) hwBE(0x%08x) hwPSW(0x%04x)\n", td_list, le32_to_cpup(&td_list->hwINFO),  le32_to_cpup(&td_list->hwCBP),  le32_to_cpup(&td_list->hwNextTD), le32_to_cpup(&td_list->hwBE), le16_to_cpup(&td_list->hwPSW[0]));

	{
		char *test;
		int i;
		printk("            content: ");
		for(i=0, test=(char*)(KSEG1ADDR(td_list->data_dma)); i<8; i++)
		{
			printk("%02x ", test[i]);
		}
		printk("\n");
...
static int rh_send_irq (ohci_t * ohci, void * rh_data, int rh_len)
{
	...
	// usb_enable_pci_slot1_frame(__LINE__);	// Added by Ed Lai

	num_ports = roothub_a (ohci) & RH_A_NDP; 
	// ----------------------------------------------- Added by Ed Lai
	// printk("[ED] num_ports (%d) MAX_ROOT_PORTS(%d) \n", num_ports ,MAX_ROOT_PORTS );
	// -----------------------------------------------
...
#if 1 /* Added by Ed Lai 2005/12/04 */
		err ("%s : %d : bogus_NDP_counter = %d, jiffies = %ld",__FUNCTION__, __LINE__, bogus_NDP_counter, jiffies);
		if  (bogus_NDP_counter++ >= MAX_bogus_NDP_counter && jiffies > SYSTEM_RUNNING_TIME ){ /* set up watch-dog to reboot the KR1 */
			err("num_ports (%d) MAX_ROOT_PORTS(%d) jiffies(%d)\n", num_ports ,MAX_ROOT_PORTS, jiffies );
			err ("bogus_NDP_counter(%d),should be reboot KR1 \n",bogus_NDP_counter );	
			{ // ************** REBOOT KR1  
...
static int hc_start (ohci_t * ohci)
{
	...
	writel (ohci->hcca_dma, &ohci->regs->hcca); /* a reset clears this */
        ...
        dbg("========================================================");
	dbg("ohci->hcca_dma :: %x ( %p )",(int) ohci->hcca_dma,  &ohci->hcca_dma);
	dbg("ohci->revision :: %x ( %p )",(int)readl (&ohci->regs->revision), &ohci->regs->revision);
	dbg("ohci->control  :: %x ( %p )",(int)readl (&ohci->regs->control),  &ohci->regs->control);
	dbg("ohci->fminterval  :: %x ( %p )",(int)readl (&ohci->regs->fminterval),  &ohci->regs->fminterval);
	dbg("========================================================");
	...
	void *mem_base, const struct pci_device_id *id)
{
	...
	//usb_dumper_endian();
	//usb_dumper_pciHead(dev);
	//printk("[ED]  %s : %s: %d \n",__FILE__,__FUNCTION__, __LINE__);

	//usb_dumper_ohci_regs(ohci);
