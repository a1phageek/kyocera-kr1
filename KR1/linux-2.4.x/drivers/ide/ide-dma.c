...
#ifdef PATCH_NEW
	stat = rtl865x_pci_ioread8(IDE_STATUS_REG);	/* get drive status */
#else
...
#ifdef PATCH_NEW
	rtl865x_pci_iowrite8(hwif->dma_status,(dma_stat & ~(1<<(5+unit))) );
#else
...
//printk("%s %s %d\n",__FILE__,__FUNCTION__,__LINE__);
	...
	/* specify r/w */
#ifdef PATCH_NEW
	rtl865x_pci_iowrite32( hwif->dma_prdtable,hwif->dmatable_dma);
	rtl865x_pci_iowrite8(hwif->dma_command,reading);
#else
...
#ifdef PATCH_NEW			
	dma_stat = rtl865x_pci_ioread8(hwif->dma_status);
#else
...
#ifdef PATCH_NEW
	rtl865x_pci_iowrite8( hwif->dma_status,dma_stat|6);
#else
...
int __ide_dma_begin (ide_drive_t *drive)
{
	ide_hwif_t *hwif	= HWIF(drive);
#ifdef PATCH_NEW		
	u8 dma_cmd		= rtl865x_pci_ioread8(hwif->dma_command);
#else
...
//printk("%s %s %d\n",__FILE__,__FUNCTION__,__LINE__);
	...
	/* start DMA */
#ifdef PATCH_NEW	
	rtl865x_pci_iowrite8( hwif->dma_command,dma_cmd|1);
#else
...
#ifdef PATCH_NEW	
	dma_cmd = rtl865x_pci_ioread8(hwif->dma_command);
#else
