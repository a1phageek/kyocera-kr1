...
{
	//printk("%c[34m%s %s %d%c[0m\n",27,__FILE__,__FUNCTION__,__LINE__,27);
...
static int actual_try_to_identify (ide_drive_t *drive, u8 cmd)
{
	...
	if (IDE_CONTROL_REG) {
		...
		ide_delay_50ms();
#ifdef PATCH_NEW 		
		a = rtl865x_pci_ioread8(IDE_ALTSTATUS_REG);
		s = rtl865x_pci_ioread8(IDE_STATUS_REG);
#else
	...
	if ((cmd == WIN_PIDENTIFY))
		/* disable dma & overlap */
#ifdef PATCH_NEW	
		rtl865x_pci_iowrite8( IDE_FEATURE_REG,0);
#else
	...
	if (hwif->identify != NULL) {
		...
		/* ask drive for ID */
#ifdef PATCH_NEW			
		rtl865x_pci_iowrite8( IDE_COMMAND_REG,cmd);
#else
	...
	do {
		...
		ide_delay_50ms();
#ifdef PATCH_NEW		
	} while ((rtl865x_pci_ioread8(hd_status)) & BUSY_STAT);
#else
	...
	ide_delay_50ms();
#ifdef PATCH_NEW		
	if (OK_STAT((rtl865x_pci_ioread8(IDE_STATUS_REG)), DRQ_STAT, BAD_R_STAT)) {
#else
	if (OK_STAT((hwif->INB(IDE_STATUS_REG)), DRQ_STAT, BAD_R_STAT)) {
		...
		/* clear drive IRQ */
#ifdef PATCH_NEW				
		(void) rtl865x_pci_ioread8(IDE_STATUS_REG);
#else
...
static int try_to_identify (ide_drive_t *drive, u8 cmd)
{
	...
	if (IDE_CONTROL_REG && !hwif->irq) {
		...
		/* enable device irq */
#ifdef PATCH_NEW					
		rtl865x_pci_iowrite8( IDE_CONTROL_REG,drive->ctl);
#else
	...
	if (autoprobe) {
		...
		/* mask device irq */
#ifdef PATCH_NEW							
		rtl865x_pci_iowrite8( IDE_CONTROL_REG,drive->ctl|2);
#else
...
#ifdef PATCH_NEW
	if (rtl865x_pci_ioread8(IDE_SELECT_REG) != drive->select.all) {
#else
...
#ifdef PATCH_NEW	
	if (rtl865x_pci_ioread8(IDE_SELECT_REG) != drive->select.all && !drive->present) {
#else
...
#ifdef PATCH_NEW
	if (OK_STAT((rtl865x_pci_ioread8(IDE_STATUS_REG)), READY_STAT, BUSY_STAT) ||
#else
	if (OK_STAT((hwif->INB(IDE_STATUS_REG)), READY_STAT, BUSY_STAT) ||
...
#ifdef PATCH_NEW		
		if (rtl865x_pci_ioread8(IDE_STATUS_REG) == (BUSY_STAT|READY_STAT))
#else
		...
		if ((rc == 1 && cmd == WIN_PIDENTIFY) &&
			...
			printk("%s: no response (status = 0x%02x), "
				"resetting drive\n", drive->name,
#ifdef PATCH_NEW				
				rtl865x_pci_ioread8(IDE_STATUS_REG));
#else
				hwif->INB(IDE_STATUS_REG));
#endif
			ide_delay_50ms();
#ifdef PATCH_NEW							
			rtl865x_pci_iowrite8( IDE_SELECT_REG,drive->select.all);
#else
			hwif->OUTB(drive->select.all, IDE_SELECT_REG);
#endif
			ide_delay_50ms();
#ifdef PATCH_NEW							
			rtl865x_pci_iowrite8( IDE_COMMAND_REG,WIN_SRST);
#else
			...
			timeout = jiffies;
#ifdef PATCH_NEW			
			while (((rtl865x_pci_ioread8(IDE_STATUS_REG)) & BUSY_STAT) &&
#else				
...
#ifdef PATCH_NEW			
			printk("%s: no response (status = 0x%02x)\n",			
				drive->name,rtl865x_pci_ioread8(IDE_STATUS_REG));
...
static void enable_nest (ide_drive_t *drive)
{
	...
	ide_delay_50ms();
#ifdef PATCH_NEW	
	rtl865x_pci_iowrite8( IDE_COMMAND_REG,EXABYTE_ENABLE_NEST);
#else
...
#ifdef PATCH_NEW		
	if (!OK_STAT((rtl865x_pci_ioread8(IDE_STATUS_REG)), 0, BAD_STAT)) {
...
static int wait_not_busy(ide_hwif_t *hwif, unsigned long timeout)
{
	...
	while(timeout--) {
		...
		mdelay(1);
#ifdef PATCH_NEW		
		stat = rtl865x_pci_ioread8(hwif->io_ports[IDE_STATUS_OFFSET]);
#else
...
int ide_wait_hwif_ready(ide_hwif_t *hwif)
{
	...
	SELECT_DRIVE(&hwif->drives[0]);
#ifdef PATCH_NEW	
	rtl865x_pci_iowrite8( hwif->io_ports[IDE_CONTROL_OFFSET],8);
#else
...
#ifdef PATCH_NEW				
		rtl865x_pci_iowrite8( hwif->io_ports[IDE_CONTROL_OFFSET],12);
#else
...
void probe_hwif (ide_hwif_t *hwif)
{
	...
	//if(memcmp(hwif->name,"ide3",4)!=0) return;
	if (hwif->noprobe)
		return;
...
#ifdef PATCH_NEW			
			rtl865x_pci_iowrite8( hwif->io_ports[IDE_CONTROL_OFFSET],0x08);
#else
