...
static char * pdc202xx_info (char *buf, struct pci_dev *dev)
{
	...
	unsigned long bibma  = pci_resource_start(dev, 4)|0xb0000000;
	...
	u8 c0	= rtl865x_ioread8(bibma + 0x02);
	u8 c1	= rtl865x_ioread8(bibma + 0x0a);
	u8 sc11	= rtl865x_ioread8(bibma + 0x11);
	u8 sc1a	= rtl865x_ioread8(bibma + 0x1a);
	u8 sc1b	= rtl865x_ioread8(bibma + 0x1b);
	u8 sc1c	= rtl865x_ioread8(bibma + 0x1c); 
	u8 sc1d	= rtl865x_ioread8(bibma + 0x1d);
	u8 sc1e	= rtl865x_ioread8(bibma + 0x1e);
	u8 sc1f	= rtl865x_ioread8(bibma + 0x1f);
...
#if PATCH_NEW
	CLKSPD = rtl865x_pci_ioread8(hwif->dma_master + 0x11);
#else
...
#if PATCH_NEW
		rtl865x_pci_iowrite8( (hwif->dma_master + 0x11),CLKSPD & ~mask);
#else
...
//		unsgned long high_16	= pci_resource_start(dev, 4);
		unsigned long high_16   = hwif->dma_master|0xb0000000;

		unsigned long atapi_reg	= high_16 + (hwif->channel ? 0x24 : 0x20);
		u32 word_count	= 0;

#if PATCH_NEW
		u8 clock = rtl865x_pci_ioread8(high_16 + 0x11);
#else
		u8 clock = hwif->INB(high_16 + 0x11);
#endif

//printk("=== %s %s %d\n",__FILE__,__FUNCTION__,__LINE__);
//printk("enter %s %s %d high_16=%x\n",__FILE__,__FUNCTION__,__LINE__,high_16); 
#if PATCH_NEW
		rtl865x_pci_iowrite8( high_16+0x11, clock|(hwif->channel ? 0x08 : 0x02));
#else
...
#if PATCH_NEW
		rtl865x_pci_iowrite32(atapi_reg,word_count);
#else
...
//		unsigned long high_16	= pci_resource_start(hwif->pci_dev, 4);
		...
		unsigned long atapi_reg	= high_16 + (hwif->channel ? 0x24 : 0x20);
		u8 clock		= 0;
//printk("=== %s %s %d high_16=%x\n",__FILE__,__FUNCTION__,__LINE__,high_16);

#if PATCH_NEW
		rtl865x_pci_iowrite32( atapi_reg,0); /* zero out extra */
		clock = rtl865x_pci_ioread8(high_16 + 0x11);
		rtl865x_pci_iowrite8(high_16+0x11,clock & ~(hwif->channel ? 0x08:0x02));
#else
...
//	unsigned long high_16	= pci_resource_start(dev, 4);
	unsigned long high_16	= hwif->dma_master;
#if PATCH_NEW
	u8 dma_stat		= rtl865x_pci_ioread8(hwif->dma_status);
	u8 sc1d			= rtl865x_pci_ioread8((high_16 + 0x001d));
#else
...
//	unsigned long high_16	= hwif->dma_base - (8*(hwif->channel));
	...
	unsigned long high_16	= pci_resource_start(hwif->pci_dev, 4)|0xb0000000;
#endif /* CONFIG_BLK_DEV_IDEDMA */

#if PATCH_NEW
	u8 udma_speed_flag	= rtl865x_pci_ioread8(high_16|0x001f);
#else
...
#if PATCH_NEW
	rtl865x_pci_iowrite8( (high_16|0x001f),(udma_speed_flag | 0x10));
	mdelay(100);
	rtl865x_pci_iowrite8( (high_16|0x001f),(udma_speed_flag & ~0x10));
...
//	unsigned long high_16	= hwif->dma_base - (8*(hwif->channel));
	unsigned long high_16	= hwif->dma_master;
#if PATCH_NEW
	u8 sc1f			= rtl865x_pci_ioread8(high_16|0x001f);
#else
...
#if PATCH_NEW
	if (state) {
		rtl865x_pci_iowrite8( (high_16|0x001f),sc1f | 0x08);
	} else {
		rtl865x_pci_iowrite8( (high_16|0x001f),sc1f & ~0x08);
	}
#else
...
//printk("=== %s %s %d\n",__FILE__,__FUNCTION__,__LINE__);
#if 0
	//dev->resource[PCI_ROM_RESOURCE].start= 0x1be10000;
	dev->resource[PCI_ROM_RESOURCE].start= 0x1bf00000;
...
//printk("=== %s %s %d\n",__FILE__,__FUNCTION__,__LINE__);
	...
	if (hwif->pci_dev->device != PCI_DEVICE_ID_PROMISE_20246) {
		...
		//printk("=== %s %s %d dma_range=%x~%x\n",__FILE__,__FUNCTION__,__LINE__,hwif->ide_dma_begin,hwif->ide_dma_end);		
	} 
...
//printk("=== %s %s %d dmabase=%x\n",__FILE__,__FUNCTION__,__LINE__,dmabase);		
		return;
	}

#if PATCH_NEW
//	printk("######## dmabase=%x\n",dmabase);
	udma_speed_flag = rtl865x_pci_ioread8(dmabase|0x1f);
#else
	udma_speed_flag	= hwif->INB((dmabase|0x1f));
#endif

#if PATCH_NEW
	primary_mode =	rtl865x_pci_ioread8(dmabase|0x1a);
#else	
...
#if PATCH_NEW
	secondary_mode = rtl865x_pci_ioread8(dmabase|0x1b);
#else	
...
#if PATCH_NEW
	rtl865x_pci_iowrite8((dmabase|0x1f),udma_speed_flag|1);
	printk("%sACTIVE\n",
			(rtl865x_pci_ioread8(dmabase|0x1f)&1) ? "":"IN");
#else
...
#if PATCH_NEW
		rtl865x_pci_iowrite8( (dmabase|0x1a),primary_mode|1);
		printk("%s\n",
			(rtl865x_pci_ioread8((dmabase|0x1a)) & 1) ? "MASTER" : "PCI");
#else
...
#if PATCH_NEW
		rtl865x_pci_iowrite8( (dmabase|0x1b),secondary_mode|1);
		printk("%s\n",
			(rtl865x_pci_ioread8((dmabase|0x1b)) & 1) ? "MASTER" : "PCI");
#else
...
static void __init init_setup_pdc20265 (struct pci_dev *dev, ide_pci_device_t *d)
{
	...
	if ((dev->bus->self) &&
		...
		printk( "ide: Skipping Promise PDC20265 "
...
static int pdc202xx_ide_init(void)
{
	...
	//big endian
	//REG32(0xbbd40064)=0x00412431;
	//little endian
	//REG32(0xbbd40064)=0x31244100;	
//printk("=== %s %s %d\n",__FILE__,__FUNCTION__,__LINE__);
