...
static unsigned int serial_in(struct async_struct *info, int offset)
{
	...
	// Debug msg. Ed Lai 2005/06/30
        //if ( offset == UART_RX && info->iomem_base +(offset<<info->iomem_reg_shift) == 0xbd011000 ){
	//	printk("[ED] Addr(0x%x) Data(%.2x)..", info->iomem_base +(offset<<info->iomem_reg_shift) , value );	
	//}
				...
				int value)
{
	...
	//char   *driver_name=driver->driver_name;
	/*
        /proc/tty/driver> cat serial
         serinfo:1.0 driver:5.05c revision:2001-07-08
         0: uart:16550 port:BD011100 irq:4 baud:37037 tx:6015 rx:52 RTS|CTS|DTR
         0: uart:16550 port:BD011000 irq:3 baud:37037 tx:153 rx:184 RTS|CTS|DTR
        */
	// if ( /* offset == UART_TX && */ info->iomem_base +(offset<<info->iomem_reg_shift) == 0xbd011000 )
	// {
		//printk("[EDb] Addr(0x%x) Data(%2x) \n", info->iomem_base +(offset<<info->iomem_reg_shift) , (unsigned char)value );	
		//*(unsigned char*)(0xbd011000)=(unsigned char)value;
...
#if defined(CONFIG_SERIAL_CONSOLE) && defined(CONFIG_MAGIC_SYSRQ) // Ed Lai 2005/0629,CONFIG_MAGIC_SYSRQ is fales 
	ignore_char:
#endif
		*status = serial_inp(info, UART_LSR);
	} while ((*status & UART_LSR_DR) && (max_count-- > 0));
...
//if ( irq == 3){ printk("rs_interrupt_single(%d)...", irq);} // Ed Lai 2005/0629
...
static void rs_flush_buffer(struct tty_struct *tty)
{
	...
	info->xmit.head = info->xmit.tail = 0;
	restore_flags(flags);
	//wake_up_interruptible(&tty->write_wait); //ed2005 2.4.31
#ifdef SERIAL_HAVE_POLL_WAIT
	wake_up_interruptible(&tty->poll_wait);
#endif
	tty_wakeup(tty);
				...
				unsigned count)
{
	 ...
	 */
	{
		//struct tty_struct *tty = info->tty; // Ed Lai 2005/0629
		//printk("[ED]%s:%d: tty(%s)\n",__FUNCTION__,__LINE__, info->tty->driver.driver_name);
	}
...
static int __init serial_console_setup(struct console *co, char *options)
{
	...
	/*state->baud_base = 14318180/16;*/
	//state->baud_base = CPU_CLOCK_RATE/16;
	if (doflow)
