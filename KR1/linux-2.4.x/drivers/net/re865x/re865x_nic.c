 ...
 * All rights reserved.                                                    
 * 
 * $Header: /home/cvsroot/uClinux-dist/linux-2.4.x/drivers/net/re865x/re865x_nic.c,v 1.6 2004/07/27 10:37:13 cfliu Exp $
 *
 * $Author: cfliu $
 *
...
#define assert(expr) \
        if((!expr)) {					\
        printk( "Return Failed %d ,%s,%s,line=%d\n",	\
...
#undef RE865X_NIC_DEBUG
//#define RE865X_NIC_DEBUG
...
__DRAM static  struct re865x_priv _rtl86xx_dev; 
...
static int32 rtl865x_toProtocolStack(struct rtl_pktHdr *pPkt,struct net_device *dev){
	...
	//printk("Rx%d from 0x%x, %s",pPkt->ph_rxdesc, pPkt->ph_portlist, dev->name);
...
static int  rtl865x_rx(struct rtl_pktHdr* pPkt){
	...
	#ifdef RE865X_NIC_DEBUG
	printk("dev->name = %s, pPkt->ph_vlanIdx = %d, %d\n",dev->name, pPkt->ph_vlanIdx, __LINE__);
	#endif
...
static int re865x_start_xmit (struct sk_buff *skb, struct net_device *dev){
	...
	pktHdr->ph_vlanIdx= dp->id & (RTL8651_VLAN_NUMBER-1);
	...
	//printk("Tx%d to 0x%x",pktHdr->ph_rxdesc,pktHdr->ph_portlist);
	//memDump(Mblk->m_data, 64,"");
...
#define DRV_INIT_CHECK(expr) do {\
	if((expr)!=SUCCESS){\
		printk("Error >>> initialize failed at line %d!!!\n", __LINE__);\
			return FAILED;\
	}\
}while(0)
...
static int re86xx_add_vlan(struct net_device *dev,int id,int port,char * mac_addr)
{
	...
	if ((rc = rtl8651_addVlan(dp->id))!=SUCCESS){
		printk("add vlan %d fail. err=%d\n",id, rc);
		return -1;
	}
	rc=rtl8651_assignVlanMacAddress(dp->id,(ether_addr_t*)mac,1);
	if(rc!=SUCCESS){
		printk("assign MAC for vlan %d fail. err=%d\n",id, rc);
		return -1;
	}
	...
	if(rc!=SUCCESS){
		printk("assign FDB for vlan %d fail.  err=%d\n",id, rc);
		return -1;
	}
	...
	for(i=0; i<MAX_PORT_NUM; i++)
	{
		if(dp->portmask& (1<<i))	{
			...
			if(rc!=SUCCESS){
				printk("assign port member %d for vlan %d fail. err=%d\n",i, id, rc);
				return -1;
			}			
			rc=rtl8651_setPvid(i, dp->id);
			if(rc!=SUCCESS){
				printk("assign PVID for vlan %d fail. err=%d\n",id, rc);
				return -1;
			}			
			rc=rtl8651_setEthernetPortAutoNegotiation(i,1,RTL8651_ETHER_AUTO_100FULL);
			if(rc!=SUCCESS){
				printk("assign autoneg for port %d fail. err=%d\n",i, rc);
				return -1;
			}
			rc=rtl8651_setEthernetPortDuplexMode(i,1);			
			if(rc!=SUCCESS){
				printk("assign duplex mode for port %d fail. err=%d\n",i, rc);
				return -1;
			}			
		}
	}
...
int  __init re865x_probe (void)
{
	...
	ether_addr_t mac;
	printk("Probing RTL8651 10/100 NIC...\n");
    	REG32(CPUIIMR) = 0x00;
	...
	if(rc)
		...
		printk("IRQ %d allocated for 8650 NIC\n", ICU_NIC);
	_rtl86xx_dev.sec_count=jiffies;
