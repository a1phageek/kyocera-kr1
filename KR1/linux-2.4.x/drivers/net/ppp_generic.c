...
// current solution is using eth0 a a bridge so, set ppp0 to the same VLAN ID
...
#undef PPP_GENERIC_DEBUG
//#define PPP_GENERIC_DEBUG

unsigned int aaa = 0;
extern int32 reCore_registerWANDevice(struct net_device* newWanDev);
extern void reCore_unregisterWANDevice(void); 
#endif
...
ppp_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	memDump(skb->data,skb->len,"ppp_start_xmit");
	memDump(skb->cb, 12, "start XMIT: CB");
	#endif
	if (strncmp(skb->cb, "FROM_ROMEDRV", 12) == 0)
	{
		#ifdef PPP_GENERIC_DEBUG
		printk("%s: isFROM_ROMEDRV, [12] = %x, [13] = %x\n",__FUNCTION__, skb->data[12], skb->data[13]);
		#endif
		
		// access bye:12 & 13
		skb->protocol = ((skb->data[12] << 8) | skb->data[13]);
		memset(skb->cb, 0, 12);
		skb->data+=14;
		skb->len-=14;
	...
	#ifdef PPP_GENERIC_DEBUG
	printk("%s npindex=%x, protocol=%x\n",__FUNCTION__,npi, ntohs(skb->protocol));
	#endif
...
ppp_do_recv(struct ppp *ppp, struct sk_buff *skb, struct channel *pch)
{
	#ifdef PPP_GENERIC_DEBUG
	printk("%s %d\n",__FUNCTION__,__LINE__);
	memDump(skb->data,skb->len,"ppp_do_recv");
	#endif
...
ppp_input(struct ppp_channel *chan, struct sk_buff *skb)
{
	...
	#ifdef PPP_GENERIC_DEBUG
	printk("%s %d\n",__FUNCTION__,__LINE__);
	memDump(skb->data,skb->len,"ppp_input");
	#endif
	if (pch == 0 || skb->len == 0) {
		kfree_skb(skb);
		return;
...
ppp_receive_frame(struct ppp *ppp, struct sk_buff *skb, struct channel *pch)
{
	#ifdef PPP_GENERIC_DEBUG
	printk("%s %d\n",__FUNCTION__,__LINE__);
	memDump(skb->data,skb->len,"ppp_receive_frame");
	#endif
	if (skb->len >= 2) {
#ifdef CONFIG_PPP_MULTILINK
...
ppp_receive_nonmp_frame(struct ppp *ppp, struct sk_buff *skb)
{
	...
	#ifdef PPP_GENERIC_DEBUG			
	printk("%s PPP_PROTO=%x\n",__FUNCTION__,proto);
	memDump(skb->data,skb->len,"ppp_recevivce-nonmp_frame");
	#endif
...
#if 1
				//memset(skb->data,0,14);
				// eth0's MAC address
				rtl865x_getWanMac(&skb->data[0]);

				// virtual gateway's MAC address
				rtl865x_getWanMac(&skb->data[6]);
				skb->data[11]++;

#endif
				skb->data[12]=0x08;// need to be real position.....!!!!!
				skb->data[13]=0;   // becareful, this must be seted to avoid uncertainty
				skb->len+=14;
			}
			
			//printk("fwd(len: %d, vid:%d, link:%d, extport:%d)\n",skb->len, CONFIG_8139CP_VID,cp->rtl8650extPortNum, cp->rtl8650linkNum);
			#ifdef PPP_GENERIC_DEBUG
			printk("%s myLinkID=%d\n",__FUNCTION__, myLinkID);
			#endif
			//retval=rtl8651_fwdEngineExtPortRecv(skb, skb->data, skb->len,10, 1<<8, myLinkID);
			retval=rtl8651_fwdEngineExtPortRecv(skb, skb->data, skb->len,8, 1<<CONFIG_RTL865XB_3G_PORT, myLinkID);

			if(retval == 0){
				//8651 fwd engine consumed the packet.
				#ifdef PPP_GENERIC_DEBUG
				printk("%s %s\n",__FUNCTION__,"8651 fwd engine consumed the packet");
				#endif
			}else if (retval==-1){
				#ifdef PPP_GENERIC_DEBUG
				printk("%s %s\n",__FUNCTION__,"exception. Drop it.");
				#endif
				//exception. Drop it.
				dev_kfree_skb_irq(skb);
			}else{
				#ifdef PPP_GENERIC_DEBUG
				printk("%s %s\n",__FUNCTION__,"to netif_rx().");
				#endif
				netif_rx(skb);
			}
		}
...
#ifdef CONFIG_RTL865XB_3G
	{
		...
		if(chipVersion[strlen(chipVersion)-1]!='B'){
			...
			goto err_unlock;
                ...
                // Since PPP0 is not a WAN device at current solution.
		//if (0!=reCore_registerWANDevice(dev))
		//	printk("XXX Can't register wan device\n");

		//if(0!=devglue_regExtDevice(dev->name, 10, 8, &myLinkID))
		if(0!=devglue_regExtDevice(dev->name, 8, CONFIG_RTL865XB_3G_PORT, &myLinkID))
			printk("XXX Can't register a link ID for device %s on extPort 0x%x!!!\n", dev->name,8 );
		else
			printk("Device %s on vlan ID %d using Link ID %d. Loopback/Ext port is %d\n", dev->name,8,  myLinkID, 8);
		//#ifdef PPP_GENERIC_DEBUG
		printk("%s myLinkID = %d, dev->name=%s\n",__FUNCTION__, myLinkID, dev->name);// can not be 0, should be 1
		//#endif
	}
#endif
...
#ifdef CONFIG_RTL865XB_3G
        // Since PPP0 is not a WAN device at current solution.
	//reCore_unregisterWANDevice();
	if(0!=devglue_unregExtDevice(myLinkID))
		printk("XXX Can't unregister link ID %d for device ppp !!!\n",myLinkID);
	else
		printk("Device ppp's main Link ID %d unregistered.\n",myLinkID);
	myLinkID=0;
#endif
