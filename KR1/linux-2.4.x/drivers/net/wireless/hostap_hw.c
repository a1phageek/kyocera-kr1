...
void dump_skb(struct sk_buff *skb)
{
	...
	printk("DUMP SKB (len:%d)\n",skb->len);
	for (i=0;i<skb->len;i+=16)
	{
		for (j=0;j<16;j++)
		{
			printk(" %02x",*(unsigned char*)&skb->data[i+j]);
		}
		printk("\n");
	}
	printk("\n");
...
*/
...
#endif /* PRISM2_PCI and PRISM2_BUS_MASTER */
	{
		...
		//TRACE(3);hostap_dump_rx_header("wlan0", (struct hfa384x_rx_frame*)skb->data);
...
/*
	TRACE(3);
	hostap_dump_rx_80211("wlan0", skb, &stats);
*/
	//TRACE(3);
	//dump_skb(skb);	
#if 0
	...
	memcpy(skb_pull(skb, rx_hdrlen - hdrlen),
		&rxdesc->frame_control, hdrlen);
