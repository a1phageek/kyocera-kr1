 ...
 * All rights reserved.
 *
 * $Id: //depot/sw/releases/linuxsrc/src/802_11/madwifi/hal/main/ar5312/ar5312_reset.c#6 $
 */
#include "opt_ah.h"
...
#undef IS
	...
	if ((IS_2413(ah) || IS_5413(ah))) {
		...
		if (bChannelChange &&
			...
			if (ar5212ChannelChange(ah, chan)) {
		                if ((ichan->channelFlags & CHANNEL_108G) == CHANNEL_108G)
                		        ar5212ArEnable(ah);
		                else if (ichan->privFlags & CHANNEL_DFS) {
	...
	macStaId1 = OS_REG_READ(ah, AR_STA_ID1) & (AR_STA_ID1_BASE_RATE_11B | AR_STA_ID1_USE_DEFANT);
	...
	if (IS_5312_2_X(ah)) {
		OS_REG_READ(ah, AR_PHY_SLEEP_SCAL);
	}
