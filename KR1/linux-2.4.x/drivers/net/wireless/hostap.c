...
void hostap_dump_rx_header(const char *name, const struct hfa384x_rx_frame *rx)
{
	...
	printk("%s: RX status=0x%04x (port=%d, type=%d, "
	...
	printk("   FC=0x%04x (type=%d:%d) dur=0x%04x seq=0x%04x "
	       "data_len=%d%s%s\n",
	...
	printk("   A1=" MACSTR " A2=" MACSTR " A3=" MACSTR " A4="
	       MACSTR "\n",
	       MAC2STR(rx->addr1), MAC2STR(rx->addr2), MAC2STR(rx->addr3),
	       MAC2STR(rx->addr4));

	printk("   dst=" MACSTR " src=" MACSTR " len=%d\n",
...
void hostap_dump_tx_header(const char *name, const struct hfa384x_tx_frame *tx)
{
	...
	printk("%s: TX status=0x%04x retry_count=%d tx_rate=%d "
