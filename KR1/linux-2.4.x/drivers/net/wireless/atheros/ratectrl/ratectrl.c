/*
 * $Id: //depot/sw/releases/SuperG_Linux_AP/src/802_11/madwifi/ratectrl/ratectrl.c#3 $
 *
...
rcRateFind(struct ath_softc *sc, struct atheros_node *pSib, A_UINT32 frameLen)
{
    ...
    */
    ...
    if (primeInUse && (ic->ic_opmode == IEEE80211_M_HOSTAP)) {
        ...
        pdevInfo->turboPrimeInfo.rateRecn = pRc->recommendedPrimeState;
#endif
#endif
    }
...
rcUpdate(struct ath_softc *sc, struct ath_node *an,
    A_BOOL Xretries, int txRate, int retries, A_RSSI rssiAck, A_UINT8 curTxAnt)
{
    struct ieee80211com	 *ic	    = &sc->sc_ic;
    struct atheros_node	*pSib       = ATH_NODE_ATHEROS(an);
    struct atheros_softc *asc	    = (struct atheros_softc *) sc->sc_rc;
    ...
    if (sc->sc_dturbo && ( (sc->sc_curmode ==  IEEE80211_MODE_TURBO_A) || (sc->sc_curmode ==  IEEE80211_MODE_TURBO_G))) 
    {
