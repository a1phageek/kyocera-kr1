...
EXPORT_SYMBOL(ath_phyerr_reset);
...
EXPORT_SYMBOL(ath_phyerr_attach);
...
#define	SM(_v, _f)	(((_v) << _f##_S) & _f)
	...
	if ((sc->sc_phyerr_cap & (ATH_RADAR_AR_EN | ATH_RADAR_EN)) &&
		...
		if (mode == IEEE80211_MODE_TURBO_G) {
			...
			val |= SM(pe->pe_curRadar->rad_radarRssi, AR_PHY_RADAR_0_RRSSI);
		} else {
			ath_reset_radar(sc);
			index = get_radar_chan_index(sc);
			pe->pe_curRadar = &pe->pe_radarState[index];
			...
			val |= SM(pe->pe_curRadar->rad_firpwr, AR_PHY_RADAR_0_FIRPWR);

			/* XXX - need to Set firpwr */
			pe->pe_curRadar->rad_radarRssi = ATH_RADAR_RRSSI;
			val |= SM(pe->pe_curRadar->rad_radarRssi, AR_PHY_RADAR_0_RRSSI);
			pe->pe_curRadar->rad_height = ATH_RADAR_HEIGHT;
			val |= SM(pe->pe_curRadar->rad_height, AR_PHY_RADAR_0_HEIGHT);
			pe->pe_curRadar->rad_pulseRssi = ATH_RADAR_PRSSI;
			val |= SM(pe->pe_curRadar->rad_pulseRssi, AR_PHY_RADAR_0_PRSSI);
			pe->pe_curRadar->rad_inband = ATH_RADAR_INBAND;
			val |= SM(pe->pe_curRadar->rad_inband, AR_PHY_RADAR_0_INBAND);
		}
		rfilt = ath_hal_getrxfilter(sc->sc_ah);
		rfilt |= HAL_RX_FILTER_PHYRADAR;
		ath_hal_setrxfilter(sc->sc_ah,rfilt);
		OS_REG_WRITE(sc->sc_ah, AR_PHY_RADAR_0, val|AR_PHY_RADAR_0_ENA);
	} else {
		rfilt = ath_hal_getrxfilter(sc->sc_ah);
		rfilt &= ~HAL_RX_FILTER_PHYRADAR;
		ath_hal_setrxfilter(sc->sc_ah,rfilt);
...
EXPORT_SYMBOL(ath_phyerr_enable);
...
ath_phyerr_disable(struct ath_softc *sc)
{
	...
	ath_hal_setrxfilter(sc->sc_ah,rfilt);
	OS_REG_WRITE(sc->sc_ah, AR_PHY_RADAR_0,
		OS_REG_READ(sc->sc_ah, AR_PHY_RADAR_0) & ~AR_PHY_RADAR_0_ENA);
}

EXPORT_SYMBOL(ath_phyerr_disable);
...
EXPORT_SYMBOL(ath_phyerr_detach);
