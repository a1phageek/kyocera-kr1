 ...
 * All rights reserved.
 *
 * $Id: //depot/sw/releases/SuperG_Linux_AP/src/802_11/madwifi/hal/diag/dumpregs.c#3 $
 */
#include "diag.h"
...
usage(void)
{
	fprintf(stderr, "usage: diag [-I interface] [-abkipx]\n");
	exit(-1);
}
...
main(int argc, char *argv[])
{
	...
	if (s < 0)
		err(1, "socket");
	strncpy(atd.ad_name, "ath0", sizeof (atd.ad_name));
	what = 0;
...
ath_hal_dumpkeycache(FILE *fd, int nkeys)
{
	...
	for (entry = 0; entry < nkeys; entry++) {
		...
		if ((type & 7) == AR_KEYTABLE_TYPE_TKIP)
