 ...
 * All rights reserved.
 *
 * $Id: //depot/sw/releases/SuperG_Linux_AP/src/802_11/madwifi/hal/diag/eeprom.c#3 $
 */
#include "diag.h"
            ...
            u_int mode, const u_int16_t maxNumCtl)
{
    ...
    while ((pTestGroups[i] != 0) && (i < maxNumCtl)) {
        ...
        fprintf(fd ,"|                                                                       |\n");
        fprintf(fd ,"| CTL: 0x%02x   [ 0x%x %s", pTestGroups[i] & 0xff, pTestGroups[i], ctlType[pTestGroups[i] & 0x3]);
...
printChannelInfo2413(FILE *fd, const HAL_EEPROM *ee, const RAW_DATA_STRUCT_2413 *pRaw)
{
    ...
    for (i = 0; i < pRaw->numChannels; i++) {
	    ...
	    if (j == 1)
		fprintf(fd ,"| %2d.%02d  |", pRaw->pDataPerChannel[i].maxPower_t4 / 4,
		    (pRaw->pDataPerChannel[i].maxPower_t4 % 4) * 25);
	    else if (j > 0)
		fprintf(fd ,"|         |");
