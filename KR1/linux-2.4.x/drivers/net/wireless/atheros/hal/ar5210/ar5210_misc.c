 ...
 * All rights reserved.
 *
 * $Id: //depot/sw/releases/SuperG_Linux_AP/src/802_11/madwifi/hal/ar5210/ar5210_misc.c#3 $
 */
#include "opt_ah.h"
...
ar5210WriteAssocid(struct ath_hal *ah, const u_int8_t *bssid, u_int16_t assocId)
{
	...
	if (assocId != 0)
		ar5210EnablePSPoll(ah, AH_NULL, 0);
	else
		ar5210DisablePSPoll(ah);
}
 ...
 */
void
ar5210EnableRadarDetection(struct ath_hal *ah)
{
	struct ath_hal_5210 *ahp = AH5210(ah);

	/* Enable RXPHY interrupt. */
	OS_REG_WRITE(ah, AR_IER, 0);
	ahp->ah_maskReg |= AR_IMR_RXPHY_INT;
