 ...
 * All rights reserved.
 *
 * $Id: //depot/sw/releases/SuperG_Linux_AP/src/802_11/madwifi/hal/ar5210/ar5210_power.c#3 $
 */
#include "opt_ah.h"
 ...
 */
static void
ar5210SetPowerModeAuto(struct ath_hal *ah, int setChip, u_int16_t sleepDuration)
{
	u_int32_t val = OS_REG_READ(ah, AR_STA_ID1);

	OS_REG_WRITE(ah, AR_STA_ID1,
		(val & ~AR_STA_ID1_DEFAULT_ANTENNA) | AR_STA_ID1_PWR_SV);
	if (setChip)
		OS_REG_WRITE(ah, AR_SCR, AR_SCR_SLE | sleepDuration);
}
...
#define	POWER_UP_TIME	2000
	...
	OS_REG_WRITE(ah, AR_STA_ID1,
		val & ~(AR_STA_ID1_PWR_SV | AR_STA_ID1_DEFAULT_ANTENNA));
	return AH_TRUE;
#undef POWER_UP_TIME
}
...
HAL_BOOL
ar5210SetPowerMode(struct ath_hal *ah, HAL_POWER_MODE mode,
		int setChip, u_int16_t sleepDuration)
{
	...
	switch (mode) {
	case HAL_PM_AUTO:
		ar5210SetPowerModeAuto(ah, setChip, sleepDuration);
		break;
	case HAL_PM_AWAKE:
 ...
 */
HAL_BOOL
ar5210InitPSPoll(struct ath_hal *ah)
{
	/* NB: This function should never be called for ar5210 */
#ifdef AH_DEBUG
 ...
 *
 * Note: for ar5210, pBssid and assocId are not used and must be set to NULL/0.
 */
HAL_BOOL
ar5210EnablePSPoll(struct ath_hal *ah, u_int8_t *bssid, u_int16_t assocId)
{
	u_int32_t val;

	val = OS_REG_READ(ah, AR_STA_ID1);
	OS_REG_WRITE(ah, AR_STA_ID1,
		 val & ~(AR_STA_ID1_NO_PSPOLL | AR_STA_ID1_DEFAULT_ANTENNA));
	return AH_TRUE;
}
 ...
 */
HAL_BOOL
ar5210DisablePSPoll(struct ath_hal *ah)
{
	u_int32_t val;

	val = OS_REG_READ(ah, AR_STA_ID1);
	OS_REG_WRITE(ah, AR_STA_ID1,
		 (val & ~AR_STA_ID1_DEFAULT_ANTENNA) | AR_STA_ID1_NO_PSPOLL);
	return AH_TRUE;
}
 ...
 */
void
ar5210SetupPSPollDesc(struct ath_hal *ah, struct ath_desc *ds,
