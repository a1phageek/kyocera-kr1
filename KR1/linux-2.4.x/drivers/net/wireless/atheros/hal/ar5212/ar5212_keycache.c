 ...
 * All rights reserved.
 *
 * $Id: //depot/sw/releases/SuperG_Linux_AP/src/802_11/madwifi/hal/ar5212/ar5212_keycache.c#3 $
 */
#include "opt_ah.h"
...
ar5212ResetKeyCacheEntry(struct ath_hal *ah, u_int16_t entry)
{
	...
	if (keyType == AR_KEYTABLE_TYPE_TKIP && AH5212(ah)->ah_micEnabled) {
                       ...
                       int xorKey)
{
	...
	case HAL_CIPHER_TKIP:
		...
		/* XXX is this right if MIC is !enabled? */
		if (entry+64 >= pCap->halKeyCacheSize) {
