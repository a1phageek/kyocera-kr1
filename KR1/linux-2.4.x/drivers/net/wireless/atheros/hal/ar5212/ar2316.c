 ...
 * All rights reserved.
 *
 * $Id: //depot/sw/releases/SuperG_Linux_AP/src/802_11/madwifi/hal/ar5212/ar2316.c#1 $
 * $File: //depot/sw/releases/SuperG_Linux_AP/src/802_11/madwifi/hal/ar5212/ar2316.c $
 * $Author: sduvvuri $
 * $DateTime: 2005/02/01 14:20:56 $
 * $Change: 127401 $
 *
 */
#include "opt_ah.h"
...
#define REG_WRITE_ARRAY_DBG(regArray, column, regWr) do {                  	\
	int r;								\
	for (r = 0; r < N(regArray); r++) {				\
		OS_REG_WRITE(ah, (regArray)[r][0], (regArray)[r][(column)]);\
		WAR_6773(regWr);					\
                printk("ar2316WriteRegs  reg 0x%x val 0x%x \n",(regArray)[r][0], (regArray)[r][(column)]);\
	        OS_DELAY(500000); \
	}								\
} while (0)
#define REG_READ_ARRAY_DBG(regArray, column, regWr) do {                  	\
	int r;								\
        u_int32_t tmp;                                        \
	for (r = 0; r < N(regArray); r++) {				\
		tmp=OS_REG_READ(ah, (regArray)[r][0]);\
                printk("ar2316ReadRegs  reg 0x%x read val 0x%x expval 0x%x \n",(regArray)[r][0],tmp,(regArray)[r][(column)]);\
	        OS_DELAY(1000); \
	}								\
} while (0)
static void
ar2316WriteRegs(struct ath_hal *ah, u_int modesIndex, u_int freqIndex, int regWrites)
{
	...
	//REG_READ_ARRAY_DBG(ar5212Modes_2316, modesIndex, regWrites);
	REG_WRITE_ARRAY(ar5212Common_2316, 1, regWrites);
	//REG_READ_ARRAY_DBG(ar5212Common_2316, 1, regWrites);
	REG_WRITE_ARRAY(ar5212BB_RfGain_2316, freqIndex, regWrites);
	//REG_READ_ARRAY_DBG(ar5212BB_RfGain_2316, freqIndex, regWrites);
}
