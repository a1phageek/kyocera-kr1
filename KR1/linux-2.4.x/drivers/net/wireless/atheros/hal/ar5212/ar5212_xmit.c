 ...
 * All rights reserved.
 *
 * $Id: //depot/sw/releases/SuperG_Linux_AP/src/802_11/madwifi/hal/ar5212/ar5212_xmit.c#3 $
 */
#include "opt_ah.h"
	...
	const HAL_TXQ_INFO *qInfo)
{
	...
	if (qInfo == AH_NULL) {
		...
		qi->tqi_qflags =
			...
			;
		if (type == HAL_TX_QUEUE_BEACON) {
			qi->tqi_aifs = INIT_BEACON_AIFS;
			qi->tqi_cwmin = INIT_BEACON_CWMIN;
		} else {
...
#define	RTSCTS	(HAL_TXDESC_RTSENA|HAL_TXDESC_CTSENA)
	...
	/* XXX validate antMode */
        ...
        txPower = (txPower*2 + ahp->ah_txPowerIndexOffset );
...
struct ar5212_desc *ads = AR5212DESC(ds);
 ...
 ath_hal_printf(ah, "HAL ContTxMode %d \n",mode);
