 ...
 * All rights reserved.
 *
 * $Id: //depot/sw/releases/SuperG_Linux_AP/src/802_11/madwifi/hal/ar5212/ar5212_misc.c#3 $
 */
#include "opt_ah.h"
	...
	u_int16_t regDomain, HAL_STATUS *status)
{
	...
	if (ath_hal_eepromWrite(ah, AR_EEPROM_REG_DOMAIN, regDomain)) {
		...
		printk("%s: set regulatory domain to %u (0x%x)\n",
			__func__, regDomain, regDomain);
...
/*
 * Enable radar detection in the hardware
 */
void
ar5212EnableRadarDetection(struct ath_hal *ah)
{
	/* Enable pulsedetection */
	OS_REG_WRITE(ah, AR_PHY_RADAR_0, 
		     OS_REG_READ(ah, AR_PHY_RADAR_0) | AR_PHY_RADAR_0_ENA);
}

void
ar5212DisableRadarDetection(struct ath_hal *ah)
{
	/* Disable pulsedetection */
	OS_REG_WRITE(ah, AR_PHY_RADAR_0,
		OS_REG_READ(ah, AR_PHY_RADAR_0) & ~AR_PHY_RADAR_0_ENA);
}
	...
	u_int32_t capability, u_int32_t *result)
{
	...
	case HAL_CAP_TKIP_MIC:		/* handle TKIP MIC in hardware */
		return ahp->ah_micEnabled ? HAL_OK : HAL_ENXIO;
	...
	u_int32_t capability, u_int32_t setting, HAL_STATUS *status)
{
	...
	case HAL_CAP_TPC:
		...
		ath_hal_printf(ah, "txPowerIndexOffset %d\n",ahp->ah_txPowerIndexOffset); 
		return AH_TRUE;
#ifdef AH_USE_RADAR
	case HAL_CAP_PHYDIAG:
		/* Set the radar threshold values in the radar register */
		v = OS_REG_READ(ah, AR_PHY_RADAR_0);
		switch (capability) {
		case HAL_CAP_RADAR_FIRPWR:
			v &= ~AR_PHY_RADAR_0_FIRPWR;
			v |= SM(setting, AR_PHY_RADAR_0_FIRPWR);
			break;
		case HAL_CAP_RADAR_HEIGHT:
			v &= ~AR_PHY_RADAR_0_HEIGHT;
			v |= SM(setting, AR_PHY_RADAR_0_HEIGHT);
			break;
		case HAL_CAP_RADAR_RRSSI:
			v &= ~AR_PHY_RADAR_0_RRSSI;
			v |= SM(setting, AR_PHY_RADAR_0_RRSSI);
			break;
		case HAL_CAP_RADAR_PRSSI:
			v &= ~AR_PHY_RADAR_0_PRSSI;
			v |= SM(setting, AR_PHY_RADAR_0_PRSSI);
			break;
		case HAL_CAP_RADAR_INBAND:
			v &= ~AR_PHY_RADAR_0_INBAND;
			v |= SM(setting, AR_PHY_RADAR_0_INBAND);
			break;
		default:
			return AH_FALSE;
		}
		OS_REG_WRITE(ah, AR_PHY_RADAR_0, v);
		return AH_TRUE;
#endif
	default:
