 ...
 * All rights reserved.
 *
 * $Id: //depot/sw/releases/SuperG_Linux_AP/src/802_11/madwifi/hal/ar5212/ar5212_reset.c#3 $
 */
#include "opt_ah.h"
...
#undef IS
	...
	if (!ar5212SetPowerMode(ah, HAL_PM_AWAKE, AH_TRUE, 0))
		return AH_FALSE;
 ...
 */
HAL_BOOL
ar5212SetXrMode(struct ath_hal *ah, HAL_OPMODE opmode)
{
#define MAC_CLKS(_usec) ((_usec) * 40)
	OS_REG_WRITE(ah, AR_XRMODE,
		  (OS_REG_READ(ah, AR_XRMODE) &~ (XR_POLL_TYPE|XR_POLL_SUBTYPE))
		| SM(FRAME_DATA, XR_POLL_TYPE)
		| SM(SUBT_NODATA_CFPOLL, XR_POLL_SUBTYPE)
	);
	/*
	...
	if (opmode == HAL_M_HOSTAP) {
		...
		OS_REG_WRITE(ah, AR_XRTO,
			  (OS_REG_READ(ah, AR_XRTO) &~
				(AR_XRTO_CHIRP_TO|AR_XRTO_POLLTO))
			| SM(MAC_CLKS(2 * XR_SLOT_DELAY +
				XR_DATA_DETECT_DELAY + 80),
				AR_XRTO_CHIRPTO)
			| SM(MAC_CLKS((INIT_AIFS_XR + INIT_CWMIN_XR) *
...
#define IQ_CAL_TRIES    10
	...
	if (ahp->ah_bIQCalibration == IQ_CAL_RUNNING &&
		...
		if (iCoffDenom != 0 && qCoffDenom >= 2) {
			...
			qCoff = ((int32_t)powerMeasI / qCoffDenom) - 128;
			...
			ichan->iCoff = (int8_t)iCoff;
			ichan->qCoff = (int8_t)qCoff;
		}
	...
	if ((IS_CHAN_5GHZ(chan) || (IS_5112(ah) && IS_CHAN_OFDM(chan))) &&
	    !IS_2413(ah))
...
ar5212SetCompRegs(struct ath_hal *ah)
{
	...
	OS_REG_WRITE(ah, MAC_DCCFG,
	     ((pDev->staConfig.abolt & ABOLT_COMPRESSION) == ABOLT_COMPRESSION));

	OS_REG_WRITE(ah, MAC_CCFG,
		(pDev->staConfig.compWinSize >> 8) & MAC_CCFG_WIN_M);
	OS_REG_WRITE(ah, MAC_CCFG,
		A_REG_RD(pDev, MAC_CCFG) | MAC_CCFG_MIB_INT_EN);
	OS_REG_WRITE(ah, MAC_CCUCFG,
		MAC_CCUCFG_RESET_VAL | MAC_CCUCFG_CATCHUP_EN);

	OS_REG_WRITE(ah, MAC_CPCOVF, 0);

	/* reset decompression mask */
	for (i = 0; i < DECOMP_MASK_SIZE; i++) {
		OS_REG_WRITE(ah, MAC_DCM_A, i);
		OS_REG_WRITE(ah, MAC_DCM_D, pDev->decompMaskArray[i]);
	}
} 
#endif
...
#define COEF_SCALE_S 24
	 ...
	 */
	coef_scaled = ((5 * (clockMhz << COEF_SCALE_S)) / 2) / chan->channel;
