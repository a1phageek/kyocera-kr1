 ...
 * All rights reserved.
 *
 * $Id: //depot/sw/releases/SuperG_Linux_AP/src/802_11/madwifi/hal/ar5211/ar5211_reset.c#3 $
 */
#include "opt_ah.h"
...
{
	if (!ar5211SetPowerMode(ah, HAL_PM_AWAKE, AH_TRUE, 0))
		return AH_FALSE;
	/*
...
ar5211SetChannel(struct ath_hal *ah,  HAL_CHANNEL_INTERNAL *chan)
{
	...
	chanIEEE = ath_hal_mhz2ieee(chan->channel, chan->channelFlags);
	if (IS_CHAN_2GHZ(chan)) {
...
getNoiseFloorThresh(struct ath_hal *ah, HAL_CHANNEL *chan, int16_t *nft)
{
	...
	case CHANNEL_OFDM|CHANNEL_5GHZ:
