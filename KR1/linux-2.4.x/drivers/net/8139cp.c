...
struct cp_private *dbg_cp;


#define cpr8(reg)	readb((u32)cp->regs + (reg))
#define cpr16(reg)	readw((u32)cp->regs + (reg))
#define cpr32(reg)	readl((u32)cp->regs + (reg))
#define cpw8(reg,val)	writeb((val), (u32)cp->regs + (reg))
#define cpw16(reg,val)	writew((val), (u32)cp->regs + (reg))
#define cpw32(reg,val)	writel((val), (u32)cp->regs + (reg))
#define cpw8_f(reg,val) do {			\
	writeb((val), (u32)cp->regs + (reg));	\
	readb( (u32)cp->regs + (reg));		\
	} while (0)
...
#define cpw32_f(reg,val) do {			\
	writel((val), (u32)cp->regs + (reg));	\
	readl( (u32)cp->regs + (reg));		\
	} while (0)
...
static void  cp_rx (struct cp_private *cp)
{
	...
	//printk("%s:%d\t  Rcvfrom %d\n", __FUNCTION__, __LINE__, rx_tail);
	while (rx_work--) {
		u32 status, len;
		dma_addr_t mapping;
...

		{
			...
			retval=rtl8651_fwdEngineExtPortRecv(skb, skb->data, skb->len, CONFIG_8139CP_VID, 1<<cp->rtl8650extPortNum, cp->rtl8650linkNum);
			if(retval==SUCCESS){
				...
				//pkt is WLAN->WLAN(maybe a host on WDS link), deliver it to bridge module.
			        skb->protocol = eth_type_trans(skb, cp->dev);
			        skb->dev = cp->dev; /* important, crash if not correct */
			        netif_rx(skb);
			}
		}
...
static void cp_tx (struct cp_private *cp)
{
	...
	//printk("%s:%d\t  Reclaim from  %d\n", __FUNCTION__, __LINE__, tx_tail);
...
static int cp_close (struct net_device *dev)
{
	...
	netif_stop_queue(dev);
#if defined(CONFIG_8139CP_HWACCEL) && defined(CONFIG_RTL865X_MULTILAYER_BSP)	
	if(0!=devglue_unregExtDevice( cp->rtl8650linkNum))
		printk("XXX Can't unregister link ID %d for device %s !!!\n",cp->rtl8650linkNum, dev->name);
	else
		printk("Device %s's main Link ID %d unregistered.\n", dev->name,cp->rtl8650linkNum);
	cp->rtl8650linkNum=0;
#endif
				  ...
				  const struct pci_device_id *ent)
{
	...
	if (rc){
		printk("Failed when init %s. Err=%d\n", dev->name, rc);
		goto err_out_free;
	}
