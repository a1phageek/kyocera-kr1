/*
 *	$Id: pci.c,v 1.10 2004/10/04 10:55:22 cfliu Exp $
 *
...
// ? ----------------------- ed 2005 -------------------------------------- ?
static void pci_dumper_pciHead (struct pci_dev *dev){
	int i,j; 
	// 0-255 => 0-248(249)
...
int pci_setup_device(struct pci_dev * dev)
{
	...
	dev->class = class;
	class >>= 8;
	pci_dumper_pciHead(dev);
	printk("Found %02x:%02x [%04x/%04x] %06x %02x\n", dev->bus->number, dev->devfn, dev->vendor, dev->device, class, dev->hdr_type);
#if 0 
//Verify PCI Config Read/Write Fucntion
		{
		u8 b;
		u16 w;
		u32 dw;
		pci_write_config_byte(dev,0x10,0x12);
		pci_read_config_byte(dev,0x10,&b);
		pci_read_config_dword(dev,0x10,&dw);
		printk("======read byte 0=%x\n",b);		
		printk("======read dword 0=%x\n",dw);	
		pci_write_config_byte(dev,0x11,0x34);
		pci_read_config_byte(dev,0x11,&b);
		pci_read_config_dword(dev,0x10,&dw);
		printk("======read byte 1=%x\n",b);		
		printk("======read dword 0=%x\n",dw);	
		pci_write_config_byte(dev,0x12,0x56);
		pci_read_config_byte(dev,0x12,&b);
		pci_read_config_dword(dev,0x10,&dw);
		printk("======read byte 2=%x\n",b);
		printk("======read dword 0=%x\n",dw);	
		pci_write_config_byte(dev,0x13,0x78);
		pci_read_config_byte(dev,0x13,&b);
		pci_read_config_dword(dev,0x10,&dw);
		printk("======read byte 3=%x\n",b);		
		printk("======read dword 0=%x\n",dw);	
		pci_write_config_word(dev,0x10,0x1234);
		pci_read_config_word(dev,0x10,&w);
		pci_read_config_dword(dev,0x10,&dw);
		printk("======read word 0=%x\n",w);		

		printk("======read dword 0=%x\n",dw);	
		
		pci_write_config_word(dev,0x12,0x5678);
		pci_read_config_word(dev,0x12,&w);
		pci_read_config_dword(dev,0x10,&dw);
		printk("======read word 2=%x\n",w);		
				printk("======read dword 0=%x\n",dw);	

		pci_write_config_dword(dev,0x10,0x12345678);		
		pci_read_config_dword(dev,0x10,&dw);
		printk("======read dword 0=%x\n",dw);	
		}
#endif
