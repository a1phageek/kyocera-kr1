 ...
 * prefer.
 */
__IRAM int handle_IRQ_event(unsigned int irq, struct pt_regs * regs, struct irqaction * action)
{
	int status;
	int cpu = smp_processor_id();
 ...
 * handlers).
 */
__IRAM asmlinkage unsigned int do_IRQ(int irq, struct pt_regs *regs)
{
	/*
