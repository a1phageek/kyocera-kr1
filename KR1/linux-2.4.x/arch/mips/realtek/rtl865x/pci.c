...
// --------------------------------- Added for EVDO Card , Ed Lai 2005/05/13 

#define PCI_CARDBUS_SUPPORT

// jiffies more than 500  -- middle boot 
// jiffies more than 1500 -- boot all most OK
#define PCI_BUS_ACTIVE 1000		// Added by Ed 2005/0705

#define PCI_EVDO_FRAME 0x00020000 // Realtek , Tony , GPIO_C4_WLAN_LED
//#define PCI_EVDO_FRAME 0x00040000 // GST, GPIO_B1_DIRECT_LED

#ifdef PCI_CARDBUS_SUPPORT

	int ghost_at_pci_slot_0 = 0; // 0: none ; 1: yes   
...
if ((address&0x1fff0000)==0x1bd40000 && jiffies > PCI_BUS_ACTIVE )
	printk("[ED]touch PCI:%s:%d:jiffies(%d) addr(0x%x)\n",__FUNCTION__,__LINE__,jiffies,(address&0x1fff0000));

#ifdef PCI_CARDBUS_SUPPORT
	REG32(0xbd01200c)&=(~PCI_EVDO_FRAME); // configured C3 as GPIO pin
	REG32(0xbd012010)|=PCI_EVDO_FRAME; // configured GPIO C3 as output pin
	if((address&0x1fff0000)==0x1bd40000)
		REG32(0xbd012014)|=PCI_EVDO_FRAME; // configured GPIO C3 as data1 , FRAME# always pull high
	else
		REG32(0xbd012014)&=(~PCI_EVDO_FRAME);// configured GPIO C3 as data0 , FRAME# link to CARDBUS
#endif

	if(!rtl865x_isLinuxIncompliantEndianMode)
	{
		ret = le32_to_cpu(REG32(address));		
	}
	else
		ret =  REG32(address);
...
//--------------------------------------------------
#ifdef PCI_CARDBUS_SUPPORT // Added by Ed Lai

// Clean up EVDO Card GHOST at PCI slot 0 when PCI Slot 0 is Empty
	if( i==0 && vendor != 0x168c ) {
		printk(" GHOST at PCI slot 0 , vendor(0x%x)\n", vendor );	
		ghost_at_pci_slot_0 = 1;
		continue;
	}	
...
//--------------------------------------------------
	...
	printk("Never Found GHOST at PCI slot 1 \n");

{
	u32 BaseAddr;
	u32 HeadType;

	HeadType=rtlpci_read_config_endian_free(config_base+PCI_CONFIG_CACHE);
	HeadType=(HeadType>>16)&0xff;

	printk("headType=%x\n",HeadType);

	if(HeadType==2)	//CardBus
	{		
		u32 temp;

		dev_function=0;
		BaseAddr = config_base+0x10; //CardBus Socket/ExCA base address
		rtlpci_write_config_endian_free(BaseAddr,0x1bf00000);
//		BaseAddr = config_base+0x44; //CardBus Socket/ExCA base address
//		rtlpci_write_config_endian_free(BaseAddr,0x1be00000);


#if 0
		BaseAddr = config_base+0x3c; //CardBus Socket/ExCA base address
		temp = rtlpci_read_config_endian_free(BaseAddr);		
		printk("config_base+0x3e=%x\n",temp);
		rtlpci_write_config_endian_free(BaseAddr,0x05400105);	
		temp = rtlpci_read_config_endian_free(BaseAddr);		
		printk("config_base+0x3e=%x\n",temp);
#endif		
		...
		pci_slot[i][dev_function]->Header_Type = 2;
...
//		scan_resource(pci_slot[i][dev_function], i, dev_function, config_base);  //probe resource request
		pci_slot[i][dev_function]->BAR[i].Size = 4096;
		pci_slot[i][dev_function]->BAR[i].Type = MEM_SPACE;

		pci_slot[i][dev_function]->BAR[i].Width = WIDTH32;  //bus width 32
		pci_slot[i][dev_function]->BAR[i].Address=0x1bf00000;
		pci_slot[i][dev_function]->BAR[i].Prefetch = 1; //prefetchable
    	    	//prefetchable
//   	    	if (data&0x0008) pci_slot[i][dev_function]->BAR[i].Prefetch = 1; //prefetchable
//   	    	else pci_slot[i][dev_function]->BAR[i].Prefetch = 0;  //no prefetch
	  ...
	  {
	  	int match=1;
		//printk("--------- RTL865XB verC and above : %s : %d \n", __FUNCTION__, __LINE__ );
...
#ifdef PCI_CARDBUS_SUPPORT
			printk("Found EVDO Card, function=%d!\n",dev_function);		
#else
...
#ifdef PCI_CARDBUS_SUPPORT // Ed Lai
// print mesg have some truble, should be corrected It
	int jj;
	REG32(0xbd01200c)&=(~PCI_EVDO_FRAME); // configured C3 as GPIO pin
	REG32(0xbd012010)|=PCI_EVDO_FRAME; // configured GPIO C3 as output pin
	printk("======================================================================\n");
	for(jj=0;jj<=4;jj++){
		...
		REG32(0xbd012014)|=PCI_EVDO_FRAME; // FRAME# link to CARDBUS
		printk("pull high 1 sec , led unlight , frame pass, cardbus on , at %x \n",jj);mdelay(300);

		REG32(0xbd012014)&=(~PCI_EVDO_FRAME); // EVDO FRAME# Break out
		printk("pull low  1 sec , led light , frame by pass, ath0   on , at %x \n",jj);mdelay(300);
		printk("----------------------------------------------------------------------\n");
	}
	printk("======================================================================\n");
#endif
...
if ((addr&0x1fff0000)==0x1bd40000 && jiffies > PCI_BUS_ACTIVE )
	printk("[ED]touch PCI:%s:%d:jiffies(%d) addr(0x%x)\n",__FUNCTION__,__LINE__,jiffies,(addr&0x1fff0000));
	
#ifdef PCI_CARDBUS_SUPPORT
	REG32(0xbd01200c)&=(~PCI_EVDO_FRAME); // configured C3 as GPIO pin
	REG32(0xbd012010)|=PCI_EVDO_FRAME; // configured GPIO C3 as output pin
	if((addr&0x1fff0000)==0x1bd40000)
		REG32(0xbd012014)|=PCI_EVDO_FRAME; // configured GPIO C3 as data1 , FRAME# always pull high
	else
		REG32(0xbd012014)&=(~PCI_EVDO_FRAME);// configured GPIO C3 as data0 , FRAME# link to CARDBUS
#endif
...
static int rtlpci_read_config_word(struct pci_dev *dev, int where, u16 *value)
{
	...
	u32 tmp,addr;
	addr = pci_slot[PCI_SLOT(dev->devfn)][PCI_FUNC(dev->devfn)]->Config_Base;
	printk(" rtlpci_read_config_word addr=%x where=%x\n",addr,where);
...
#ifdef PCI_CARDBUS_SUPPORT	
// Fix me : ghost at pci slot 0 so dont need assign 
	if(ghost_at_pci_slot_0 != 1){	
		printk("================= Sync to Rewrite It =============== \n");
		//REG32(0xbbd40010)=0x1bf00000;
		REG32(0xbbd80010)=0x1bf10000;
		//REG32(0xbbd80110)=0x1bf11000;
		printk("================= Sync to Rewrite It =============== \n");
	}
#endif
