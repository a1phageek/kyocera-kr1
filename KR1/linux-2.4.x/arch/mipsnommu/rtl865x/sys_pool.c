/*
 *	$Id: sys_pool.c,v 1.2 2004/02/27 02:55:16 cfliu Exp $
 *
...
void *consistent_alloc(int gfp, size_t size, dma_addr_t *dma_handle)
{
	...
	ret =(void*) ((u32)(__ioremap(virt_to_phys(virt), size, 0))|0xa0000000);
#if 1
...
#ifdef CONFIG_RTLPOOL_DEBUG
	printk ("rtlpool create  size %d, %d/page (%d alloc)\n",
		size, retval->blocks_per_page, allocation);
#endif
 ...
 * @pool: rtl pool that will be destroyed
 *
...
rtl_pool_destroy (struct rtl_pool *pool)
{
...
#ifdef CONFIG_RTLPOOL_DEBUG
	printk (KERN_DEBUG "rtlpool destroy \n");
#endif
	...
	while (!list_empty (&pool->page_list)) {
		...
		if (is_page_busy (pool->blocks_per_page, page->bitmap)) {
			printk (KERN_ERR "rtl_pool_destroy %s, %p busy\n",
 ...
 * @pool: rtl pool that will produce the block
...
/**
 * rtl_pool_free - put block back into rtl pool
 * @pool: the rtl pool holding the block
...
rtl_pool_free (struct rtl_pool *pool, void *vaddr, dma_addr_t dma)
{
	...
	if ((page = pool_find_page (pool, dma)) == 0) {
		printk (KERN_ERR "rtl_pool_free %s  %p/%x (bad dma)\n",
			pool->name, vaddr, (int) (dma & 0xffffffff));
		return;
	}
#ifdef	CONFIG_RTLPOOL_DEBUG
	if (((dma - page->dma) + (void *)page->vaddr) != vaddr) {
		printk (KERN_ERR "rtl_pool_free %s  %p (bad vaddr)/%x\n",
			pool->name, vaddr, (int) (dma & 0xffffffff));
		return;
	}
#endif
	...
	if (page->bitmap [map] & (1UL << block)) {
		printk (KERN_ERR "rtl_pool_free %s  dma %x already free\n",
			pool->name, dma);
		return;
	}
	if (pool->flags & SLAB_POISON)
