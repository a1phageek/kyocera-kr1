...
{ 
  u8 tmp8;
	...
	/* Soft reset the chip. */
	Reg8w(ioaddr+ChipCmd, (Reg8r(ioaddr+rChipCmd)&ChipCmdClear) | CmdReset);
	while (Reg8r(ioaddr+rChipCmd)&CmdReset) ;  //wait reset complete
	/* Bring the chip out of low-power mode. */	
	Reg8w(ioaddr+Config1, 0);
	/* make sure chip thinks PIO and MMIO are enabled */
	tmp8 = Reg8r(ioaddr+rConfig1);
	if ((tmp8 & Cfg1_PIO) == 0) {
...
u32 rtl8139nic_Open(void)
{
  ...
  u8 tmp;
    ...
    for (i=0;i<4;i++) {
      REG32(ioaddr+TxAddr0+i*4) = (u32)(txbfr+i*2048)&0x1FFFFFFF;  //map to physical addr
    };
    ...
    REG32(ioaddr+RxBuf) = (u32)rxbfr&0x1FFFFFFF;  //map to physical addr
    
	/* Soft reset the chip. */
	Reg8w(ioaddr+ChipCmd, (Reg8r(ioaddr+rChipCmd)&ChipCmdClear) | CmdReset);
	while (Reg8r(ioaddr+rChipCmd)&CmdReset) ;  //wait reset complete
	/* Must enable Tx/Rx before setting transfer thresholds! */
	Reg8w(ioaddr+ChipCmd, (Reg8r(ioaddr+rChipCmd) & ChipCmdClear) | CmdRxEnb | CmdTxEnb);
        //RX config
        if (RXBFRSIZE==8192)
            REG32(ioaddr+RxConfig) = RX_MODE;//0xFF00E00F;
        else
            REG32(ioaddr+RxConfig) = 0x0000E90F;//0xFF00E00F;
        //reset CARP
        Reg16w(ioaddr+RxBufPtr, 0x0000FFF0);
    	//TX config
	REG32(ioaddr+TxConfig) = TX_MODE;//0x00010000;

	/* unlock Config[01234] and BMCR register writes */
	Reg8w(ioaddr+Cfg9346, Cfg9346_Unlock);
	for (i=0;i<100;i++);
	//config 
    //auto flow control
    //Reg8w(ioaddr+rMediaStatus, 0xc0);        
    //auto neg
    Reg16w(ioaddr+BasicModeCtrl, 0x1100);        
	tmp = Reg8r(ioaddr+rConfig1) & Config1Clear;
	tmp |= Cfg1_Driver_Load;
	tmp |= 3; /* Enable PM/VPD */
	Reg8w(ioaddr+Config1, tmp);
	tmp = Reg8r(ioaddr+rConfig4) & ~(1<<2);
	/* chip will clear Rx FIFO overflow automatically */
	tmp |= (1<<7);
	Reg8w(ioaddr+Config4, tmp);
	/* disable magic packet scanning, which is enabled
	 * when PM is enabled above (Config1) */
	Reg8w(ioaddr+Config3, Reg8r(ioaddr+rConfig3) & ~(1<<5));
	/* lock Config[01234] and BMCR register writes */
	Reg8w(ioaddr+Cfg9346, Cfg9346_Lock);
	for (i=0;i<100;i++);
	
	//reset miss counter
	REG32(ioaddr+RxMissed) = 0;
	/* no early-rx interrupts */
	Reg16w(ioaddr+MultiIntr, Reg16r(ioaddr+rMultiIntr) & MultiIntrClear);
	/* make sure RxTx has started */
	Reg8w(ioaddr+ChipCmd, (Reg8r(ioaddr+rChipCmd)& ChipCmdClear) | CmdRxEnb | CmdTxEnb);
	//reset Tx index
	tx_producer = 3;
	/* Enable all known interrupts by setting the interrupt mask. */
	Reg16w(ioaddr+IntrMask, rtl8139_intr_mask);
	//Reg16w(ioaddr+IntrMask, 0);
...
{
    //disable Rx
    Reg8w(ioaddr+ChipCmd, CmdTxEnb);
    
    //reset counter
    *loc=0;
    Reg16w(ioaddr+RxBufPtr, 0xFFF0);
    //set Rx mode
    REG32(ioaddr+RxConfig) = 0x0000E10F;//0xFF00E00F;
    
    //re-enable Rx
    Reg8w(ioaddr+ChipCmd, (Reg8r(ioaddr+rChipCmd)& ChipCmdClear) | CmdRxEnb | CmdTxEnb);
    while ((Reg8r(ioaddr+rChipCmd)&(CmdRxEnb|CmdTxEnb))!=(CmdRxEnb|CmdTxEnb) ) ;
...
void reset_tx(void)
{
    ...
    //set Tx mode again
    REG32(ioaddr+TxConfig) = TX_MODE;//0xFF00E00F;
...
void wait_TXOK(u32 ioaddr, u32 TxStatus)
{
  u32 status;
    
#if 1
wait_pause:
        status = Reg8r(ioaddr+rMediaStatus);  //get pause flag
        if (status&0x00000001) goto wait_pause;
wait_txok:
        //status = Reg16r(ioaddr+rIntrStatus);
        //if (status&0x2020) 
        //    Reg16w(ioaddr+IntrStatus, 0x2020);
  //for (i=0;i<0x80000;i++) {
  //};
        status = REG32(ioaddr+TxStatus);
        //printk("1 %X\r",status);
...
//        if (status&0x2020) {
//          if (status!=0) {
//            Reg16w(ioaddr+IntrStatus, status);
//            goto check_sts;
//        };
#endif        
};	
...
wait1:
        status = Reg16r(ioaddr+rTxSummary);
        //printk("1 %X\r",status);
        ...
        if (i<60) i=60;
        if (i>1510) i=1510;
        REG32(ioaddr+TxStatus0) = 0x00FF0000|size;  //size=256
        if (i==1510) i=60;
...
wait2:
        ...
        if (i<60) i=60;
        if (i>1510) i=1510;
        REG32(ioaddr+TxStatus0+4) = 0x00FF0000|size;  //size=256
        if (i==1510) i=60;
...
wait3:
        ...
        if (i<60) i=60;
        if (i>1510) i=1510;
        REG32(ioaddr+TxStatus0+8) = 0x00FF0000|size;  //size=256
        if (i==1510) i=60;
...
wait4:
        ...
        if (i<60) i=60;
        if (i>1510) i=1510;
        REG32(ioaddr+TxStatus0+12) = 0x00FF0000|size;  //size=256
        if (i==1510) i=60;
    };	
}
...
waitRx:
    ...
    if (tmp_sts!=0)  //clear status, rx bfr/fifo overflow not included
      Reg16w(ioaddr+IntrStatus, tmp_sts);
    if (!(status&0x0003)) goto waitRx;

    //clear rx pkt related status only, we need to handle overflow case
    Reg16w(ioaddr+IntrStatus, status&0x03);
    
    hw_count=Reg16r(ioaddr+rRxBufAddr);
...
wait_tx_ready:
        ...
        REG32(ioaddr+TxStatus0+tmp*4) = 0x003F0000|(pkt_size-4);  //send it
//wait_tx_ready1:        
//        status = Reg16r(ioaddr+rTxSummary);
//            for (i=0;i<100;i++) ;
//        if (!(status&(1<<tmp))) goto wait_tx_ready1;
    ...
    kk++;
    //update CAPR
    Reg16w(ioaddr+RxBufPtr, (consumer-16)&(RXBFRSIZE-1));
    if(ovfl_sts!=0) {  //clear overflow case
      //reset_rx((u32*)&consumer);
      Reg16w(ioaddr+IntrStatus, ovfl_sts);
      ovfl_sts = 0;
    };
    getApkt--;
    goto repeat_test;	
...
int *init_RTL8139nic(int slot, int func, pci_config_s *pcfg)
{
  ...
  u8 bfr[8];
     ...
     txbfr=0;
     rxbfr=0;
    ioaddr = pcfg->BAR[1].Address;
    getApkt = 0;
    /* Put the chip into low-power mode. */
    Reg8w(ioaddr+Cfg9346, Cfg9346_Unlock);
    for (i=0;i<100;i++); 
    i=Reg8r(ioaddr+rCfg9346);

    /*reset phy*/
    Reg16w(ioaddr+BasicModeCtrl, (Reg16r(ioaddr+rBasicModeCtrl)&0x7FFF) | 0x8000);
    while ((Reg16r(ioaddr+rBasicModeCtrl)&0x8000)) ;  //wait reset complete
    
    //LED on/OFF
    tmp = Reg8r(ioaddr+rConfig1);
    Reg8w(ioaddr+Config1, tmp&0x3F);
    Reg8w(ioaddr+Config1, tmp|0xc0);
    Reg8w(ioaddr+Config1, tmp&0x3F);
...
int rtl8139nic_ISR(pci_config_s *pcfg)
{
  ...
  int rtn_val;
	...
	if (status & RxUnderrun)
		link_changed = Reg16r(ioaddr+rCSCR8139) & CSCR_LinkChangeBit;
	//clear IRQ
  	Reg16w(ioaddr+IntrStatus, status);
	...
	if (status & RxOK) {	/* Rx interrupt, loopback */
	    if (!(Reg8r(ioaddr+rChipCmd)&1)) {  //pkt in Rx bfr
	        //u16 rx_size;
	        getApkt += 1;
	        //disable RX
 	        //Reg8w(ioaddr+0x0037, Reg8r(ioaddr+rChipCmd)&0xF7);
	        //tmp=(tx_producer+1)&3;
	        //rx_size=*((u16*)rxbfr+14);
	        //copy pkt data from Rx bfr to Tx bfr
	        //memcpy(txbfr+tmp*2048, rxbfr+2, rx_size+18);
	        //memcpy(txbfr+tmp*2048, rxbfr, rx_size+18);
	        //REG32(ioaddr+TxStatus0+tmp*4) = REG32(ioaddr+TxStatus0+tmp*4)|rx_size;  //send it
	        //REG32(ioaddr+TxStatus0+tmp*4) = REG32(ioaddr+TxStatus0+tmp*4)|0x100;  //send it
	    	//tx_producer = tmp;  //update pointer
		    rtn_val= -1;
	    };
	};
	...
	if (status & (TxOK | TxErr)) {  //enable rx
	    if (REG32(ioaddr+TxStatus0)&0x00002000) TxWait[0]=0;
	    else  TxWait[0]=1;
	    if (REG32(ioaddr+TxStatus0+4)&0x00002000) TxWait[1]=0;
	    else  TxWait[1]=1;
	    if (REG32(ioaddr+TxStatus0+8)&0x00002000) TxWait[2]=0;
	    else  TxWait[2]=1;
	    if (REG32(ioaddr+TxStatus0+12)&0x00002000) TxWait[3]=0;
	    else  TxWait[3]=1;
		rtn_val= -1;
	};
    return rtn_val;
