...
static void sp720_flush_page_to_ram(struct page * page)
{
}

/* TX39H-style cache flush routines. */
static void sp720_flush_icache_all(void)
{
#if 0
	volatile int *p = 0x88000000;
	int i;
	int j;
	for(i=0;i<256;i++)
	{
...
#endif
   unsigned long reg=0,reg_tmp=0;

#if DISABLE
   return;
#endif   
	/* $20 of coprocessor0 is the cache control register of sunplus's MIPS*/

   __asm__ __volatile__ ("mfc0\t %0,$20\n\t"
           :"=r" (reg)
           :"r" (reg)
   ) ;
   reg_tmp = reg&~0x2;      //set bit 0 for invalid data cache
...
void sp720_flush_dcache_all(void)
{
   unsigned long reg=0,reg_tmp=0;
   ...
   reg_tmp = reg&~0x1;      //set bit 0 for invalid data cache
   __asm__ __volatile__ (
           "mtc0\t %0,$20\n\t"
           :
           :"r" (reg_tmp)
...
static void sp720_flush_cache_all(void)
{
	sp720_flush_icache_all();
	sp720_flush_dcache_all();
}


void __init ld_mmu_simulator(void)
	
{

	_clear_page = r3k_clear_page;
	_copy_page = r3k_copy_page;
	_flush_cache_all	= sp720_flush_cache_all;
	___flush_cache_all	= sp720_flush_cache_all;
	_flush_cache_mm		= (void *) sp720_flush_dcache_all;
	_flush_cache_range	= (void *) sp720_flush_dcache_all;
	_flush_cache_page	= (void *) sp720_flush_dcache_all;
	_flush_cache_sigtramp	= (void *) sp720_flush_cache_all;
	_flush_page_to_ram	= sp720_flush_cache_all;
//sp720_flush_page_to_ram;
	_flush_icache_page	= (void *) sp720_flush_icache_all;
	_flush_icache_range	= (void *) sp720_flush_icache_all;
...
void __init simulator_tlb_init(void)
{
	//local_flush_tlb_all();
	//todo init vector for utlb? memcpy((void *)KSEG0, &except_vec0_r2300, 0x80);
	//flush_icache_range(KSEG0, KSEG0 + 0x80);
}

#include <asm/bootinfo.h>

void simulator_init_boot_mem_map()
{
//xipos NO_MM mem
	boot_mem_map.nr_map = 1;
	boot_mem_map.map[0].addr = SDRAM_BASE;
	boot_mem_map.map[0].type = BOOT_MEM_RAM;
	boot_mem_map.map[0].size = 16*1024*1024;
