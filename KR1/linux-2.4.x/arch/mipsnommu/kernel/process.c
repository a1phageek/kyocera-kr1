...
#ifdef RTL865X_DEBUG 
//See /proc/utilization...
#include <linux/proc_fs.h>
static unsigned long  Counter,PreviousTime;
unsigned  loopCount[CPU_UTILIZATION_HISTORY];
unsigned maxLoopCount;
ATTRIB_NORET void cpu_idle(void)
{
	int s;
	unsigned  CurrentTime, largestCounter;
	for(s=0;s<CPU_UTILIZATION_HISTORY;s++){
		loopCount[s]=0;
	}
	largestCounter = Counter =maxLoopCount= 0;
	/* endless idle loop with no priority at all */
	current->nice = 20;
	current->counter = -100;
	init_idle();
	PreviousTime=jiffies;
	while (1) {
		while (!current->need_resched){
			{
#define RISR	0xbd012004
#define RIMR	(*((volatile unsigned *)0xbd012000))

				CurrentTime=jiffies;
				if(CurrentTime -  PreviousTime < 300)
					//update cpuLoad[] every 3 seconds.					
					Counter++;
				else {
					unsigned flag, isr = RISR & RIMR;
					clear_cp0_status( STATUSF_IP2| STATUSF_IP3| STATUSF_IP4| STATUSF_IP5|
						STATUSF_IP6| STATUSF_IP7);
					flag = RIMR;
					RIMR=0;
					for(s=CPU_UTILIZATION_HISTORY-1; s>0;s--) {
						loopCount[s] = loopCount[s-1];
					}
					if(maxLoopCount < Counter)
						maxLoopCount = Counter;
					loopCount[0]=Counter;
					Counter = 0;
					PreviousTime=jiffies;
