...
/* allow gdb to handle lbc timeout interrupt */
static struct gdb_regs lbc_regs;
static char lbc_happened = 0;
void gdb_stub_backupRegs(struct pt_regs *regs)
{
    /* copy main processor registers */
    memcpy(&lbc_regs.reg0, regs->regs, sizeof(lbc_regs));
    /* copy coprocessor 0 registers */
    lbc_regs.cp0_status = regs->cp0_status;
    lbc_regs.lo = regs->lo;
    lbc_regs.hi = regs->hi;
    lbc_regs.cp0_epc = regs->cp0_epc;
    lbc_regs.cp0_badvaddr = regs->cp0_badvaddr;
    lbc_regs.cp0_cause = regs->cp0_cause;
...
void handle_exception (struct gdb_regs *regs)
{
    ...
    if (lbc_happened)
    {
        memcpy(regs, &lbc_regs, sizeof(lbc_regs));
        lbc_happened = 0;
    }
	...
	*ptr++ = 0;
	...
	while (1) {
		...
		case 'q':
			...
			strcpy(output_buffer, "Text=0;Data=0;Bss=0");
			break;
		case '?':
			output_buffer[0] = 'S';
