...
extern asmlinkage void handle_dummy(void);
...
{
	show_regs(regs);
	traceStack(regs->regs[31], regs->regs[29],regs->regs[31] );
...
asmlinkage void do_dummy(struct pt_regs *regs)
{
}
...
#error "ll/sc emulation is not SMP safe"
	...
	//printk(__FUNCTION__"() epc=0x%x op=0x%x\n", regs->cp0_epc, opcode);
		if ((opcode & OPCODE) == LL) {
		...
		#define GETRS(op)	((op&0x3e00000)>>21)
			...
			{
			case 0x18: // MULT
				regs->regs[GETRD(opcode)] = mul_32_32_signed(regs->regs[GETRS(opcode)], regs->regs[GETRT(opcode)],
					       	&(regs->hi), &(regs->lo));
				regs->cp0_epc+=4;
				return;
			case 0x19: // MULTU
				regs->regs[GETRD(opcode)] = mul_32_32_unsigned(regs->regs[GETRS(opcode)], regs->regs[GETRT(opcode)],
					       	&(regs->hi), &(regs->lo));
				regs->cp0_epc+=4;
				return;
			case 0x1a: // DIV
				div_32_32_signed(regs->regs[GETRS(opcode)], regs->regs[GETRT(opcode)],&(regs->lo), &(regs->hi));
				regs->cp0_epc+=4;
				return;
			case 0x1b: // DIVU
				div_32_32_unsigned(regs->regs[GETRS(opcode)], regs->regs[GETRT(opcode)],&(regs->lo), &(regs->hi));
				regs->cp0_epc+=4;
				return;
			}
		}
...
#endif
//xipos
#ifdef NO_MM	 /*rupert */ /* exception handler */
...
#else
	/* FIXME, zxl */
	////#define SIMULATOR_EXCEPTION	KSEG0 /*0x88000000+16*1024*1024-1024*/
	#define SIMULATOR_EXCEPTION (KSEG0)
	clear_cp0_status (ST0_BEV);
	/* Copy the generic exception handler code to it's final destination. */
	memcpy((void *)(SIMULATOR_EXCEPTION + 0x80), &except_vec1_generic, 0x80);
	memcpy((void *)(SIMULATOR_EXCEPTION + 0x100), &except_vec2_generic, 0x80);
	memcpy((void *)(SIMULATOR_EXCEPTION + 0x180), &except_vec3_generic, 0x80);
#endif
	/*
	...
	#ifdef NO_MM
	simulator_irq_setup();  // for simulator
	...
	//set_except_vector(9, handle_dummy);
...
#else
	flush_icache_range(SIMULATOR_EXCEPTION,SIMULATOR_EXCEPTION+0x400);
#endif
	if (mips_cpu.options & MIPS_CPU_FPU) {
