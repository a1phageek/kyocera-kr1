...
{
	/* patch tx39_intr_table */
	int i;
#if 0 /* rupert*/
	printk(__FUNCTION__"()\n");

	/* Clear and Disable All Interrupts */
	WRITE_TX39REG(0, OFF_INTR_ENABLE1);
	WRITE_TX39REG(0, OFF_INTR_ENABLE2);
	WRITE_TX39REG(0, OFF_INTR_ENABLE3);
	WRITE_TX39REG(0, OFF_INTR_ENABLE4);
	WRITE_TX39REG(0, OFF_INTR_ENABLE5);
	WRITE_TX39REG(0, OFF_INTR_ENABLE6);

	WRITE_TX39REG(0xffffffff, OFF_INTR_CLEAR1);
	WRITE_TX39REG(0xffffffff, OFF_INTR_CLEAR2);
	WRITE_TX39REG(0xffffffff, OFF_INTR_CLEAR3);
	WRITE_TX39REG(0xffffffff, OFF_INTR_CLEAR4);
	WRITE_TX39REG(0xffffffff, OFF_INTR_CLEAR5);


	set_except_vector(0, sim_handle_int);

	/* Enable TX39 Global interrupt */
	WRITE_TX39REG(INTR_GLOBALEN, OFF_INTR_ENABLE6);
...
static unsigned int tx39_intr_table[GVPDA_INTR_MAXIRQ][TX39_MAXINTR_REG] = {
	{0, 0, 0, 0, INTR_PERINT, 0},		/* 0. Interval Timer */
	{0, INTR_UARTATXINT|INTR_UARTARXINT|INTR_UARTARXOVERRUNINT
		|INTR_UARTAFRAMEERRINT|INTR_UARTABREAKINT
		|INTR_UARTAPARITYERRINT,0,
		0, 0, 0},					/* 1. uart */
	{0, 0, 0, 0, INTR_POSONBUTNINT|INTR_NEGONBUTNINT|
         INTR_POSPWRINT|INTR_NEGPWRINT|INTR_NEGPWROKINT, 0},    /* 2. power */
	{INTR_SIBIRQPOSINT|INTR_SIBIRQNEGINT, 0, 0, 0, 0, 0},   /* 3: AFC Interrupt */
	{0, 0, 0, 0, 0, 0},					/* 4. */
	{INTR_SND0_5INT | INTR_SND1_0INT, 0, 0, 0, 0, 0},	/* 5. sound DMA*/
	{0, 0, 0, 0, 0, 0},					/* 6. */
	{0, 0, 0, 0, 0, 0},					/* 7. */
	{0, 0, 0, 0, 0, 0},					/* 8. */
	{0, 0, 0, 0, 0, 0},					/* 9. */
	{0, 0, 0, 0, 0, 0},					/* a  */
	//{INTR_SIBSF0INT|INTR_SIBSF1INT, 0, 0, 0, 0, 0},		/* b: sib related */
	{0, 0, 0, 0, 0, 0},					/* b: sib related */
	{0, 0, 0, 0, INTR_RTCINT|INTR_ALARMINT, 0},		/* C. RTC & Alarm*/  // colin : 20010411
	{0, 0, 0, 0, INTR_IOPOSINT_MASK, 0},			/* D. GPIO button */
	{0,INTR_UARTBRXINT|INTR_UARTBTXINT
        |INTR_UARTBRXOVERRUNINT|INTR_UARTBTXOVERRUNINT, 0, 0, 0, 0},  /* E. IRDA */
	{0, 0, INTR_MFIOPOSINT(1), INTR_MFIONEGINT(1), 0, 0}	/* F. USB plugin & put */
};

extern void lx4180_timer_interrupt(int irq, void *dev_id, struct pt_regs *regs);
void sim_handle_interrupt(struct pt_regs *regs)
{
	int i;
	unsigned short masked_intr;
	unsigned int irq = 0;
	unsigned int intren_off_addr = OFF_INTR_ENABLE1;
	unsigned int intrcs_off_addr = OFF_INTR_STATUS1;
	

	masked_intr=REG16(GISR);
	for (i=0;i<10;i++)
	{
...
#if 0 /* rupert*/
	for (i=0; i < TX3911_MAXINTR_REG; i++)
	{
		masked_intr = READ_TX39REG(intren_off_addr) & READ_TX39REG(intrcs_off_addr);
		intren_off_addr += 4;
		intrcs_off_addr += 4;
		...
		for (irq=0; irq<GVPDA_INTR_MAXIRQ; irq++) {
			if (tx39_intr_table[irq][i] & masked_intr) {
