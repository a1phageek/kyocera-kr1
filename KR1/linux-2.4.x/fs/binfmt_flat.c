...
#ifdef NO_MM
#undef MAX_SHARED_LIBS
#define MAX_SHARED_LIBS 0
#endif
...
#if 0
void waitdo(int regs,int start_addr,int stack)
{
	DBG_FLT("start_thread(regs=0x%x, entry=0x%x, start_stack=0x%x)\n",
		regs, start_addr, stack);
}
#endif
#ifdef CONFIG_BINFMT_SHARED_FLAT
...
#define TOP_OF_ARGS (PAGE_SIZE * MAX_ARG_PAGES - sizeof(void *))
	...
	/*waitdo((int)regs, (int)start_addr, (int)current->mm->start_stack);*/
	start_thread(regs,
		     start_addr,
