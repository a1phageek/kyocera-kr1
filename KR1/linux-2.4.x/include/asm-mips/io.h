...
static __inline__ u8 rtl865x_ioread8(u32 addr)
{
	...
	if((addr&0x1be00000)==0x1be00000)
		rval= rtl865x_pci_ioread8(addr);
	else
#endif	
		rval=(*(volatile unsigned char *)(addr));
	rmb();
	return rval;
#else
...
#ifdef CONFIG_PCI
	if((addr&0x1be00000)==0x1be00000)
		rval= rtl865x_pci_ioread16(addr);
	else
#endif	
	rval= __ioswab16((*(volatile unsigned short *)(addr)));
	rmb();
	return rval;
#else
...
#ifdef CONFIG_PCI
	if((addr&0x1be00000)==0x1be00000)
		rval= rtl865x_pci_ioread32(addr);
	else
#endif	
	rval= __ioswab32((*(volatile unsigned int *)(addr)));
	rmb();
	return rval;
#else
...
static inline unsigned short inw(unsigned long port)
{
	...
	unsigned short w;
	unsigned long addr;
//	printk("inw_before(0x%08x)\n",(unsigned int)port);
	if (port & 0x2)
		addr = port & (~0x2);
	else
		addr = port |0x2;
//	printk("inw_after(0x%08x)\n",(unsigned int) addr);
	w = *(volatile u16 *)addr;
	//printk("=%04x\n", w);
	return w;
...
#if 0
	unsigned long l;
	//printk("inl(0x%08x)", (unsigned int)(port));
	l = (*(volatile u32 *)(port));
	return l;
#else
