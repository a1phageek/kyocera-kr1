...
/*
#if UMTS
void get_csq(char buf[])
{
	FILE *f, *f1;
	int count, signal, constant;
	
	count = 0;
	signal = 0;
	constant = 0;

	f = fopen("/var/ppp/CSQ", "r");
	if( f != NULL)
	{
		printf("[Penguin-chat.c] opening file /var/ppp/CSQ\n");
		count = fscanf(f, "+CSQ: %d, %d", &signal, &constant);
		fclose(f);
		...
		printf("[Penguin-chat.c] cann't open file /var/ppp/CSQ\n");
		signal = 0;
	}
		
	f1 = fopen("/var/ppp/signal", "w");
	if(f1 != NULL)
	{
		printf("[Penguin-chat.c] opening file /var/ppp/signal\n");
		fprintf(f1, "%d", signal);
		fclose(f1);
	}else
		printf("[Penguin-chat.c] cann't open file /var/ppp/signal\n");
...
void get_csq(char buf[])
{
	...
	{
		printf("[Penguin-chat.c] opening file /var/ppp/CSQ\n");
		count = fscanf(f, "%d, %d", &signal, &constant);
		fclose(f);
...
// 2006-02-21: Peter chao add for 3G(KR1) 
	if(strstr(line, "CSQ") || strstr(line, "csq") || strstr(line, "99") && strstr(line, ","))
	{
		printf("[Penguin-chat.c] logv: AT+CSQ=%s\n", line);
		
		f = fopen("/var/ppp/CSQ", "w");
		fprintf(f, "%s\n", line);
		fclose(f);
