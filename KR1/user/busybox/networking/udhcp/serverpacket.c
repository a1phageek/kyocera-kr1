...
int sendOffer(struct dhcpMessage *oldpacket)
{
	...
	struct dhcpManualAddr *manual = NULL;
	u_int32_t req_align, lease_time_align = server_config.lease;
	unsigned char *req, *lease_time,*host_name;
	struct option_set *curr;
	struct in_addr addr;

	init_packet(&packet, oldpacket, DHCPOFFER);

	if ((manual=find_manual_by_chaddr(oldpacket->chaddr))) {
		packet.yiaddr = manual->yiaddr;
	}
...
int sendACK(struct dhcpMessage *oldpacket, u_int32_t yiaddr)
{
	struct dhcpMessage packet;
	struct option_set *curr;
	unsigned char *lease_time,*host_name;
	...
	add_lease(packet.chaddr, packet.yiaddr, lease_time_align,host_name);
