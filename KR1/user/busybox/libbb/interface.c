 ...
 *
 * Version:     $Id: interface.c,v 1.1.1.1 2003/08/27 06:20:14 rupert Exp $
 *
...
static void ife_print(struct interface *ptr)
{
	...
	char addr6p[8][5];
#endif
	char filename[32]; // Peter add for 3G
	FILE *f_3g;  // Peter add for 3G

	sprintf(filename,"/var/tmp/if.%s", ptr->name); // Peter add for 3G
//	printf("%s\n", filename);i    // Peter add for 3G
	f_3g = fopen(filename, "w");  // Peter add for 3G
...
#define IPV6_ADDR_RESERVED      0x2000U	/* reserved address space */
	...
	if (ptr->statistics_valid) {
		...
		/* Peter add for 3G*/
		fprintf(f_3g,"%Lu %Lu\n", ptr->stats.tx_packets, ptr->stats.tx_bytes);
		fprintf(f_3g,"%Lu %Lu \n", ptr->stats.rx_packets, ptr->stats.rx_bytes);
		/* Peter add for 3G end*/
		if (can_compress)
