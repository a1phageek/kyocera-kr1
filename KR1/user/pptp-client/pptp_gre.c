 ...
 *
 * $Id: pptp_gre.c,v 1.5 2004/07/08 05:45:11 tony Exp $
 */
...
#if 0
  if ( (PPTP_GRE_IS_S(ntoh8(header->flags))) && (PPTP_GRE_IS_A(ntoh8(header->ver))) )
  	printf("<-- seq=%d ack=%d\n",header->seq,header->ack);
  else if(PPTP_GRE_IS_S(ntoh8(header->flags)))
  	printf("<-- seq=%d\n",header->seq);
  else
  	printf("<-- ack=%d\n",header->seq);
#endif  	
...
#if 0
  if ( (PPTP_GRE_IS_S(u.header.flags)) && (PPTP_GRE_IS_A(u.header.ver)) )
  	printf("--> seq=%d ack=%d\n",seq_sent,ack_sent);
  else if(PPTP_GRE_IS_S(u.header.flags))
  	printf("--> seq=%d\n",seq_sent);
  else
  	printf("--> ack=%d\n",ack_sent);
#endif  
