...
int EventHandler(Upnp_EventType EventType, void *Event, void *Cookie)
{
	switch ( EventType )
	{
		...
		case UPNP_CONTROL_ACTION_REQUEST:
			...
			break;
		default:
			if (g_debug) printf( "Error in EventHandler: Unknown event type %d",
						EventType);
	}
	return (0);
}
...
int StateTableInit(char *descDocUrl)
{
	...
	{
		printf( "Could not parse descritipion document. Exiting ...");
		UpnpFinish();
		exit(0);
	}
...
int HandleSubscriptionRequest(struct Upnp_Subscription_Request *sr_event)
{
	...
	if (strcmp(sr_event->UDN, gateUDN) == 0)
	{
		...
		{
			if (g_debug) printf( "Recieved request to subscribe to WANCommonIFC1");
		...
		else if (strcmp(sr_event->ServiceId, "urn:upnp-org:serviceId:WANIPConn1") == 0)
		{
			...
			if (g_debug) printf( "Received request to subscribe to WANIPConn1");
...
int HandleGetVarRequest(struct Upnp_State_Var_Request *gv_request)
{
	...
	// please let us know.
	if (g_debug) printf( "Deprecated Get Variable Request received. Ignoring.");
	return 1;
}
...
int HandleActionRequest(struct Upnp_Action_Request *ca_event)
{
	...
	if (strcmp(ca_event->DevUDN, gateUDN) == 0)
	{
		...
		if (g_debug) printf( "ActionName = %s", ca_event->ActionName);
		...
		else if (strcmp(ca_event->ServiceID,"urn:upnp-org:serviceId:WANCommonIFC1") == 0)
		{
			...
			{
				if (g_debug) printf( "Invalid Action Request : %s",ca_event->ActionName);
...
int GetConnectionTypeInfo (struct Upnp_Action_Request *ca_event)
{
   ...
   {
      if (g_debug) printf( "Error parsing Response to GetConnectionTypeinfo: %s", resultStr);
      ca_event->ActionResult = NULL;
...
int GetNATRSIPStatus(struct Upnp_Action_Request *ca_event)
{
   ...
   sprintf(resultStr, "<u:GetNATRSIPStatusResponse xmlns:u=\"urn:schemas-upnp-org:service:WANIPConnection:1\">\n"
	...
	{
		if (g_debug) printf( "Error parsing Response to GetNATRSIPStatus: %s", resultStr);
		ca_event->ActionResult = NULL;
		ca_event->ErrCode = 402;
	}
...
int RequestConnection(struct Upnp_Action_Request *ca_event)
{
	...
	if (g_debug) printf( "RequestConnection recieved ... Setting Status to %s.", ConnectionStatus);
...
int GetCommonLinkProperties(struct Upnp_Action_Request *ca_event)
{
   ...
   {
      if (g_debug) printf( "Error parsing Response to GetCommonLinkProperties: %s", resultStr);
      ca_event->ActionResult = NULL;
      ca_event->ErrCode = 402;
   }
...
int GetTotalBytesSent(struct Upnp_Action_Request *ca_event)
{
   ...
   {
      if (g_debug) printf( "Error parsing Response to GetTotalBytesSent: %s", resultStr);
      ca_event->ActionResult = NULL;
      ca_event->ErrCode = 402;
   }
...
int GetTotalBytesReceived(struct Upnp_Action_Request *ca_event)
{
   ...
   {
      if (g_debug) printf( "Error parsing Response to GetTotalBytesReceived: %s", resultStr);
      ca_event->ActionResult = NULL;
      ca_event->ErrCode = 402;
   }
...
int GetTotalPacketsSent(struct Upnp_Action_Request *ca_event)
{
   ...
   {
      if (g_debug) printf( "Error parsing Response to GetPacketsSent: %s", resultStr);
      ca_event->ActionResult = NULL;
      ca_event->ErrCode = 402;
   }
...
int GetTotalPacketsReceived(struct Upnp_Action_Request *ca_event)
{
   ...
   {
      if (g_debug) printf( "Error parsing Response to GetPacketsReceived: %s", resultStr);
      ca_event->ActionResult = NULL;
      ca_event->ErrCode = 402;
   }
...
int GetStatusInfo(struct Upnp_Action_Request *ca_event)
{
   ...
   {
      if (g_debug) printf( "Error parsing Response to GetStatusInfo: %s", resultStr);
      ca_event->ActionResult = NULL;
      ca_event->ErrCode = 402;
   }
...
int AddPortMapping(struct Upnp_Action_Request *ca_event)
{
		...
		&& (desc = GetFirstDocumentItem(ca_event->ActionRequest, "NewPortMappingDescription") ))
	{
		...
		{
				if (g_debug) printf( "Found port map to already exist.  Replacing");
				pmlist_Delete(ret);
		}
		...
		if (result==1)
		{
			...
			if (g_debug) printf( "PortMappingNumberOfEntries: %d", pmlist_Size());
			UpnpAddToPropertySet(&propSet, "PortMappingNumberOfEntries", num);				
			UpnpNotifyExt(deviceHandle, ca_event->DevUDN, ca_event->ServiceID, propSet);
			ixmlDocument_free(propSet);
			if (g_debug) printf( "AddPortMap: RemoteHost: %s Prot: %s ExtPort: %s Int: %s.%s\n",
		...
		else
		{
			if (result==718)
			{
				if (g_debug) printf("Failure in GateDeviceAddPortMapping: RemoteHost: %s Prot:%s ExtPort: %s Int: %s.%s\n",
					remote_host, proto, ext_port, int_ip, int_port);
	...
	{
		if (g_debug) printf( "Failiure in GateDeviceAddPortMapping: Invalid Arguments!");
		ca_event->ErrCode = 402;
...
int GetGenericPortMappingEntry(struct Upnp_Action_Request *ca_event)
{
	...
	if ((mapindex = GetFirstDocumentItem(ca_event->ActionRequest, "NewPortMappingIndex")))
	{
   	...
   	{
            	if (g_debug) printf( "Failure in GateDeviceGetGenericortMappingEntry: Invalid Args");
            	ca_event->ErrCode = 402;
                strcpy(ca_event->ErrStr, "Invalid Args");
                ca_event->ActionResult = NULL;
   	}
...
int GetSpecificPortMappingEntry(struct Upnp_Action_Request *ca_event)
{
      ...
      && (proto = GetFirstDocumentItem(ca_event->ActionRequest,"NewProtocol")))
   {
      if ((strcmp(proto, "TCP") == 0) || (strcmp(proto, "UDP") == 0))
      {
         ...
         {
            if (g_debug) printf( "Failure in GateDeviceGetSpecificPortMappingEntry: PortMapping Doesn't Exist...");
      ...
      {
              if (g_debug) printf( "Failure in GateDeviceGetSpecificPortMappingEntry: Invalid NewProtocol=%s\n",proto);
                        ca_event->ErrCode = 402;
                        strcpy(ca_event->ErrStr, "Invalid Args");
                        ca_event->ActionResult = NULL;
      }
   ...
   {
      if (g_debug) printf( "Failure in GateDeviceGetSpecificPortMappingEntry: Invalid Args");
      ca_event->ErrCode = 402;
      strcpy(ca_event->ErrStr, "Invalid Args");
      ca_event->ActionResult = NULL;
   }
...
int GetExternalIPAddress(struct Upnp_Action_Request *ca_event)
{
   ...
   {
      if (g_debug) printf( "Error parsing Response to ExternalIPAddress: %s", resultStr);
      ca_event->ActionResult = NULL;
      ca_event->ErrCode = 402;
   }
...
int DeletePortMapping(struct Upnp_Action_Request *ca_event)
{
      ...
      (proto = GetFirstDocumentItem(ca_event->ActionRequest, "NewProtocol"))))
   {
     ...
     if ((strcmp(proto, "TCP") == 0) || (strcmp(proto, "UDP") == 0))
     {
         ...
         if (result==1)
         {
            if (g_debug) printf( "DeletePortMap: Proto:%s Port:%s\n",proto, ext_port);
            sprintf(num,"%d",pmlist_Size());
            UpnpAddToPropertySet(&propSet,"PortMappingNumberOfEntries", num);
            UpnpNotifyExt(deviceHandle, ca_event->DevUDN,ca_event->ServiceID,propSet);
            ixmlDocument_free(propSet);
         ...
         {
            if (g_debug) printf( "Failure in GateDeviceDeletePortMapping: DeletePortMap: Proto:%s Port:%s\n",proto, ext_port);
            ca_event->ErrCode = 714;
            strcpy(ca_event->ErrStr, "NoSuchEntryInArray");
            ca_event->ActionResult = NULL;
         }
      ...
      {
         if (g_debug) printf( "Failure in GateDeviceDeletePortMapping: Invalid NewProtocol=%s\n",proto);
         ca_event->ErrCode = 402;
			strcpy(ca_event->ErrStr, "Invalid Args");
			ca_event->ActionResult = NULL;
      }
   ...
   {
		if (g_debug) printf( "Failiure in GateDeviceDeletePortMapping: Invalid Arguments!");
		ca_event->ErrCode = 402;
		strcpy(ca_event->ErrStr, "Invalid Args");
		ca_event->ActionResult = NULL;
   }
