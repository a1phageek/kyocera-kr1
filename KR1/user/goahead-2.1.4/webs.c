 ...
 *
 * $Id: webs.c,v 1.3 2003/10/06 13:17:38 rupert Exp $
 */
...
static char_t	websRealm[64] = T("Realtek RTL8651 Router");	/* Realm name */
...
// add cjchen
		if (text != NULL)
			...
			len = wp->lenPostData;
			if (len<=(32*1024)) 		/* add by rupert*/
			{	
        	          	wp->query = brealloc(B_L, wp->query, (len +	nbytes +
	        	             1) * sizeof(char_t));
	        	        if (wp->query) {
        	          		memcpy(&wp->query[len], text, nbytes);
	                	  	wp->query[len+nbytes] = '\0';
				}
			}
			else
			{
				if (firmFileCreate!=1) /* add by rupert*/
				{
					...
					KILL("/var/run/syslogd.pid",9);
				      	fd1=fopen("/var/run.bix","w");
					for (i=0;i<len;i++)
						fputc(wp->query[i],fd1);
					fclose(fd1);
					firmFileCreate=1;
				}

			     	 fd1=fopen("/var/run.bix","a+");
			      	 fseek(fd1,0,SEEK_END);
			         fwrite(text,nbytes,1,fd1);
  			      	 fclose(fd1);
			}
