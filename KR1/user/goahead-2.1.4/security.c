 ...
 *
 * $Id: security.c,v 1.2 2003/10/06 13:17:38 rupert Exp $
 */
...
#ifdef SINGLE_USER_PASSWORD
	if (mgmtUserName())
	{
		//fprintf(stderr,"username %s\n",mgmtUserName());
		if (!userid) goto AuthFail;
		if (strcmp(userid,mgmtUserName()))
			goto AuthFail;
	}
	if (mgmtPassword())
	{
		//fprintf(stderr,"password  %s\n",mgmtPassword());
		if (!password) goto AuthFail;
		if (strcmp(password,mgmtPassword()))
			goto AuthFail;
	}
	return 0;
AuthFail:
