...
//#include "../web_server/model/cfg.h"	//Added by Nick HO For CFG_TLV
#define CFG_TLV		1	////Added by Nick HO For CFG_TLV
//#undef CFG_TLV
...
typedef struct vlanCfgParam_s
{
	...
	uint32			field_end;	//Keep it at the last location & keep for uint32 for alignment, Nick HO, 12/26/2004
} vlanCfgParam_t;
...
enum ifCfgParam_connType_e {
	...
	IFCFGPARAM_CONNTYPE_3G,    // support 3G EVDO WWAN, Nick Wang
};
...
typedef struct ifCfgParam_s	/* Interface Configuration */
{
	...
	uint32			field_end;  //Keep it at the last location, Nick HO, 12/26/2004	
} ifCfgParam_t;
...
typedef struct pptpCfgParam_s	/* PPTP Configuration */
{
	...
	uint32		field_end;		//Added by Nick HO For 4 Bytes alignment
...
typedef struct natCfgParam_s /* nat Configuration */
{
	...
	//uint8			rsv[2];			/* for 4 byte alignment */  // Marked by Nick HO
	uint32          field_end;			//Added by Nick HO for 4 byte alignment
	
} natCfgParam_t;


///howard
enum pppcfgParam_langType_e {
	PPPCFGPARAM_LANGTYPE_ENGLISH = 0,
	PPPCFGPARAM_LANGTYPE_TAIWAN,
...
#define PPPOE_DESTNET_NO	4  //Added by Nick HO
...
typedef struct pppoeCfgParam_s
{
	...
	int8			destnet[PPPOE_DESTNET_NO][32];  /* pppoe destination network route 1~4 */	
	int8			destnetType[PPPOE_DESTNET_NO];  /* pppoe destination network type 1~4 */
...
#define DEV_DOMAIN_NAME_LEN_S 	31
#define DHCPS_HWARR_NO			8
typedef struct dhcpsCfgParam_s
{
	...
	uint8      domain_name[DEV_DOMAIN_NAME_LEN_S+1];	
	uint32     dhcp_lease; // seconds
	uint8	   rsv;     	/* for 4 byte alignment */
	uint32	   manualIp[DHCPS_HWARR_NO];
	uint8	   hardwareAddr[DHCPS_HWARR_NO][6];
	uint8	   ConnType;       //eddie for 2G
	uint8 username_2g[32];	   //eddie for 2G
	uint8 password_2g[32];	   //eddie for 2G	
	uint8 server_2g[16];	   //eddie for 2G
	uint8 dial_number_2g[16];  //eddie for 2G
	uint8	   language_type;  //eddie for language
	uint32	   field_end;     	/* for 4 byte alignment */ //Added by Nick HO
} dhcpsCfgParam_t;
...
typedef struct dhcpcCfgParam_s
{
	...
	uint32       field_end; 		    /* for alignment */  //Added by Nick HO
} dhcpcCfgParam_t;
...
typedef struct dnsCfgParam_s
{
	...
	uint32			field_end;		//For 4 byte align, Nick HO
} dnsCfgParam_t;
...
typedef struct aclGlobalCfgParam_s 
{
	...
	uint32		field_end;				// for 4 bytes alignment , Added by Nick HO
} aclGlobalCfgParam_t;
...
typedef struct aclCfgParam_s 
{
    ...
    uint16      start_time;
    uint16      end_time;
    uint8       filter;		// 0: firewall, 1: IP filter
    uint32		field_end;	//Keep it at the last location, Nick HO, 12/26/2004
        // Dino Chang
} aclCfgParam_t;
...
typedef struct tbldrvCfgParam_s 
{
	...
	uint32		field_end;			//Added by Nick HO for 4 bytes align
} tbldrvCfgParam_t;
...
typedef struct ddnsCfgParam_s	/* DDNS Configuration */
{
	...
	uint32		field_end;  //For 4 bytes alignment
	
} ddnsCfgParam_t;

typedef struct pppCfgParm_s //peter and howard
{
	uint8 username[32];
	uint8 password[32];
	uint8 server[16];
	uint8 dial_number[16];

	uint32 langType;	/* connection type: 0->english, 1->taiwan, howard */
	uint32 field_end;	//Keep it at the last location, Nick HO, 12/26/2004	 
} pppCfgParam_t;  //peter and howard
...
#define	WLAN_KEYNUM		4		//Added by Nick HO
#define RAD_SERVER_NUM_S	2
#define RAD_SECRET_LEN_S	31

typedef struct {
	uint32 ip;
	uint16 port;
	int8 secret[RAD_SECRET_LEN_S+1];
	uint32			field_end;	//Keep it at the last location, Nick HO, 12/26/2004
} RADSRV_S;
...
typedef struct wlanCfgParam_s	/* Wireless LAN Configuration */
{
	...
	uint8	key[68]; //10 hex for wep64,26 hex for wep128, 64 hex for WPA-PSK, It's string see the board.c DefaultFunction (Nick HO Comment)
	/* add by Min Lee */
	uint8   auto_channel;	// 0: disable, 1: enable
	...
	uint8	key64[WLAN_KEYNUM][5];		// 64Bits -- Min Lee, It's not string
	uint8	key128[WLAN_KEYNUM][13];		// 128Bits -- Min Lee, It's not string
	uint8	passphrase[65];		// Passphrase for WPA-PSK , It's string
 	uint8	turbo;				// 0: disable, 1: without, 2: dynamic, 3: static
 	uint16  dtim;
 	/* Radius Server */
    RADSRV_S	rad_srv[RAD_SERVER_NUM_S];    // primary & secondary radius server        
 	uint32	field_end;	//Keep it at the last location, Nick HO, 12/26/2004
} wlanCfgParam_t;
...
typedef struct qosCfgParam_s /* QoS Configuration */
{
	...
	union {
		...
		struct {				//index>=8, use this structure, from board.c (Nick HO)
			uint16	port;
			uint8	isHigh;
...
#define  RATELIMIT_ENTRY_NO	16	//Added by Nick HO, 12/30/2004

typedef struct rateLimitCfgParam_s
{
	uint32		enable;		/* 0: disable, 1: enable */
	rateLimitEntry_t entry[RATELIMIT_ENTRY_NO];
	uint32		field_end;		//Added by Nick HO, for 4 Bytes alignment
} rateLimitCfgParam_t;
...
typedef struct romeCfgParam_s
{
	...
	pppCfgParam_t       pppCfgParam; ///pater and howard
	...
	uint32				tabid;		// Added by Nick HO, please keep this on the last location, for aligment issue
} romeCfgParam_t;
...
typedef struct wlanCfgParam_s	/* Wireless LAN Configuration */
{
	...
	uint8	wep_key_len;		// 1:64Bits, 2:128Bits
	uint8	wep_def_key;		// 0:key1, 1:key2, 2:key3, 3:key4
	uint8	key64[WLAN_KEYNUM][5];		// 64Bits -- Min Lee
	uint8	key128[WLAN_KEYNUM][13];		// 128Bits -- Min Lee
	uint8	passphrase[65];		// Passphrase for WPA-PSK 
 	uint8	turbo;				// 0: disable, 1: without, 2: dynamic, 3: static
 	uint16  dtim;
 	/* Radius Server */
    RADSRV_S	rad_srv[RAD_SERVER_NUM_S];    // primary & secondary radius server        
