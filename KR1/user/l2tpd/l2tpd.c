...
void death_handler (int signal)
{
    ...
    int sec;
    log (LOG_CRIT, "l2tpd - %s: Fatal signal %d received\n", __FUNCTION__, signal);
	if (signal != SIGTERM) {
		st = tunnels.head;
		while (st)
		{
...
goto_restart:
	...
	debug_tunnel = 0;
	packet_dump = 0;            /* Dump packets? */
	debug_avp = 0;              /* Debug AVP negotiations? */
	debug_state = 0;            /* Debug state machine? */

	printf("l2tpd - waiting for dial(%d)!\n",call_times);
	while(restart_flag==0)
	{
		sleep(1);
	}
	restart_flag=0;
	printf("l2tpd - connecting!\n");
	call_times++;
