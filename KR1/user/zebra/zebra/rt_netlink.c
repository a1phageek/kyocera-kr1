...
kernel_add_ipv4 (struct prefix *p, struct rib *rib)
{
  ...
  myAddr.s_addr = 0;
  int32  ret;
  char nexthop_ifname[IFNAMSIZ];
  uint32 args[4];	

  // Transform prefixlen to netmask. eg. 24 -> 255.255.255.0
  for(i = 31; i >= (31 - p->prefixlen) + 1; i--)
     myAddr.s_addr |= (1 << i);
  ...
  f1 = fopen("/var/rip.log", "a");
  if(f1)
  {
  	fprintf(f1,"==== Prefix ========\n");
  	fprintf(f1,"p->family: %d\n", p->family);
	fprintf(f1,"p->prefixlen: %d\n", p->prefixlen);
	fprintf(f1,"p->u.prefix: %d\n", p->u.prefix);
	fprintf(f1,"p->u.lp.id: %s\n", inet_ntoa(p->u.lp.id));
	fprintf(f1,"p->u.lp.adv_router: %s\n", inet_ntoa(p->u.lp.adv_router));
	fprintf(f1,"p->u.val: %s\n", p->u.val);
	fprintf(f1,"=====Rib ==========\n");
	fprintf(f1,"rib->type: %d\n", rib->type);
	fprintf(f1,"rib->nexthop_active_num: %d\n", rib->nexthop_active_num);
	fprintf(f1,"rib->nexthop_fib_num: %d\n", rib->nexthop_fib_num);
	fprintf(f1,"rib->nexthop_num: %d\n", rib->nexthop_num);
	fprintf(f1,"rib->distance: %d\n", rib->distance);
	fprintf(f1,"rib->flags: %d\n", rib->flags);
	fprintf(f1,"rib->metric: %d\n", rib->metric);
	fprintf(f1,"rib->refcnt: %ld\n", rib->refcnt);
	fprintf(f1,"rib->table: %d\n", rib->table);
	fprintf(f1,"rib->uptime: %ld\n", rib->uptime);
	fprintf(f1,"rib->nexthop->flags: %d\n", rib->nexthop->flags);
	fprintf(f1,"rib->nexthop->type: %d\n", rib->nexthop->type);
	fprintf(f1,"rib->nexthop->ifindex: %d\n", rib->nexthop->ifindex);
	fprintf(f1,"ifindex ----> ifname (if_indextoname): %s\n", if_indextoname(rib->nexthop->ifindex, ifname_tmp));
	fprintf(f1,"rib->nexthop->ifname: %s\n", rib->nexthop->ifname);
	fprintf(f1,"rib->nexthop->rtype: %d\n", rib->nexthop->rtype);
	fprintf(f1,"rib->nexthop->rifindex: %d\n", rib->nexthop->rifindex);
	fprintf(f1,"rib->nexthop->rgate.ipv4: %s\n", inet_ntoa(rib->nexthop->rgate.ipv4));
	fprintf(f1,"rib->nexthop->gate.ipv4: %s\n", inet_ntoa(rib->nexthop->gate.ipv4));
	fclose(f1);
    }
    */
    
//  rtl8651_addRoute(p->u.prefix4.s_addr, myAddr.s_addr, if_indextoname(rib->nexthop->ifindex, nexthop_ifname), rib->nexthop->gate.ipv4.s_addr);

  args[0] = (uint32)p->u.prefix4.s_addr;
  args[1] = (uint32)myAddr.s_addr;

  // Given a ifindex, 'if_indextoname' returns a pointer to the interface name.
  // We cannot use 'rib->nexthop->ifname' for the interface name, because it is always 'NULL'.
  args[2] = if_indextoname(rib->nexthop->ifindex, nexthop_ifname);
  args[3] = (uint32)rib->nexthop->gate.ipv4.s_addr;

  // The first argument of 're865xIoctl' is always 'eth0'.
