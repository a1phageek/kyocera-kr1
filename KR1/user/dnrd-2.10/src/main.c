...
int main(int argc, char *argv[])
{
    ...
    /*
     * Change our root and current working directories to /var/dnrd.
     * Also, so some sanity checking on that directory first.
     */

#ifndef EMBED
    dirp = opendir("/var/dnrd");
    if (!dirp) {
	log_msg(LOG_ERR, "The directory /var/dnrd must be created before "
		"dnrd will run");
    }

    rslt = stat("/var/dnrd", &st);
    if (st.st_uid != 0) {
	log_msg(LOG_ERR, "The /var/dnrd directory must be owned by root");
	cleanexit(-1);
    }
    if ((st.st_mode & (S_IWGRP | S_IWOTH)) != 0) {
	log_msg(LOG_ERR,
		"The /var/dnrd directory should only be user writable");
	cleanexit(-1);
    }
	...
	if (S_ISDIR(st.st_mode)) {
	    log_msg(LOG_ERR, "The /var/dnrd directory must not contain "
		    "subdirectories");
	    cleanexit(-1);
	}
	if ((st.st_mode & (S_IXUSR|S_IXGRP|S_IXOTH|S_IWGRP|S_IWOTH)) != 0) {
	    log_msg(LOG_ERR, "A file in /var/dnrd has either execute "
		    "permissions or non-user write permission.  Please do a "
		    "\"chmod a-x,go-w\" on all files in this directory");
	    cleanexit(-1);
	}
	if (st.st_uid != 0) {
	    log_msg(LOG_ERR, "All files in /var/dnrd must be owned by root");
	    cleanexit(-1);
	}
    }
    closedir(dirp);
