...
static int std_rcv_pads(struct session* ses,
    ...
    #if 1 /* hiwu */    
    {
        extern unsigned char server_mac[ETH_ALEN];
        extern unsigned int  ses_sid;
        memcpy(server_mac, p_in->addr.sll_addr, ETH_ALEN);
	ses_sid = p_in->hdr->sid;
	fprintf(stderr,"session id %x %d\n",
		ses->sp.sa_addr.pppoe.sid = p_in->hdr->sid,
		ses->sp.sa_addr.pppoe.sid = p_in->hdr->sid
);
