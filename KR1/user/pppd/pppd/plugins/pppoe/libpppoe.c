		      ...
		      int data_len)
{
    ...
    if( !pt ){
//    printf("Missing tag %d. Expected %s\n", id,data);
...
//		 id,data);
    ...
    if(len != data_len){
//    	printf("Length mismatch on tag %d: expect: %d got: %d\n",id, data_len, len);
//	poe_info(ses,"Length mismatch on tag %d: expect: %d got: %d\n",	 id, data_len, len);
	return 0;
    }

    if( 0!=memcmp(pt->tag_data,data,data_len)){
//    	printf("Tag data mismatch on tag %d: expect: %s vs %s\n", id, data,pt->tag_data);
...
#define CHECK_TAG(name, val)					\
	...
	//printf("HOST_UNIQ mismatch: %08x %i\n");
	return -1;
    }
...
//rupert  while(ses->retransmits < ses->retries || ses->retries==-1 ){
 while(1){
...
//rupert	if (ses->retransmits == ses->retries)
//		return -1;
